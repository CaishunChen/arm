...
//-----XSL-----Begin
#elif (defined(XKAP_ICARE_B_C)||defined(PROJECT_NRF5X_BLE)||defined(TEST_NRF52_V1))
#include "includes.h"
#include "Bsp_Twi.h"
#define i2c_write(a, b, c, d)    BspTwi_Write1_nByte(a,b,d,c)
#define i2c_read(a, b, c, d)     BspTwi_Read_nByte(a,b,d,c)
#define delay_ms                 MODULE_OS_DELAY_MS
#define get_ms(a)                Count_GetTimeMs((uint32_t*)a)
static inline int reg_int_cb(struct int_param_s *int_param)
{
    //sensor_board_irq_connect(int_param->pin, int_param->cb, int_param->arg);
    int_param=int_param;
