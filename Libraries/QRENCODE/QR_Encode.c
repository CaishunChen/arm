...
//uint8_t m_nBlockLength[MAX_DATACODEWORD];
uint8_t *m_byBlockMode;
uint8_t *m_nBlockLength;

int m_ncAllCodeWord;
//uint8_t m_byAllCodeWord[MAX_ALLCODEWORD];
//uint8_t m_byRSWork[MAX_CODEBLOCK];
uint8_t *m_byAllCodeWord;
uint8_t *m_byRSWork;
...
uint8_t EncodeData(char *lpsSource)
{
    ...
    m_byDataCodeWord =  &pbuf[0*MAX_DATACODEWORD];
    m_byBlockMode    =  &pbuf[1*MAX_DATACODEWORD];
    m_nBlockLength   =  &pbuf[2*MAX_DATACODEWORD];
    pbuf1=  MemManager_Get(E_MEM_MANAGER_TYPE_5KB_BASIC);
    m_byAllCodeWord  =  &pbuf1[0*MAX_ALLCODEWORD];
    m_byRSWork       =  &pbuf1[1*MAX_ALLCODEWORD];
    //
    ncSource = 0;
    m_nLevel = 0;
    m_nMaskingNo = 0;
    ...
    ncTerminater = COUNT_MIN(4, (ncDataCodeWord * 8) - m_ncDataCodeWordBit);
    ...
    for (i = 0; i < ncBlock1; ++i)
    {
        memset(m_byRSWork, 0, MAX_CODEBLOCK);
...
int EncodeSourceData(char *lpsSource, int ncLength, int nVerGroup)
{
    ...
    memset(m_nBlockLength, 0, MAX_DATACODEWORD);
    ...
    for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
    {
        ...
        else if (m_byBlockMode[i] == QR_MODE_ALPHABET)
        {
            ...
            for (j = 0; j < m_nBlockLength[i]; j += 2)
            {
                ...
                {
                    wBinCode = (uint16_t)((AlphabetToBinaly(lpsSource[ncComplete + j]) * 45) +
                ...
                {
                    wBinCode = (uint16_t)AlphabetToBinaly(lpsSource[ncComplete + j]);
        ...
        else// if(m_byBlockMode[i] == QR_MODE_KANJI)
        {
            ...
            {
                uint16_t wBinCode = KanjiToBinaly((uint16_t)(((uint8_t)lpsSource[ncComplete + (j * 2)] << 8) + (uint8_t)lpsSource[ncComplete + (j * 2) + 1]));
...
e)将结果转换为13位二进制串
*/
uint16_t ChineseToBinaly(uint16_t wc)
{
    if(wc >= 0xa1a1 && wc <= 0xa9fe)
    {
        return (uint16_t)((((wc >> 8)-0xa1) * 0x60) + ((wc & 0x00ff)-0xa1));
    }
    if(wc >= 0xb0a1 && wc <= 0xf9fe)
    {
        return (uint16_t)((((wc >> 8)-0xa6) * 0x60) + ((wc & 0x00ff)-0xa1));
    }
    return (uint16_t)((((wc >> 8)-0xa6) * 0x60) + ((wc & 0x00ff)-0xa1));
}
//纠错码
...
void Print_2DCode(void)
{
    ...
    uint8_t bit_h,bit_v,print_num;//print_buff[1024];
    uint8_t print_buff[256];
