...
void Hci_100ms(void)
{
    ...
    else if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]>6*100 && s_keyBell==2)
    {
        s_keyBell=3;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BBB,0);
    }
    else if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]>9*100 && s_keyBell==3)
    {
        s_keyBell=4;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BBBB,0);
    }
    else if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]>12*100 && s_keyBell==4)
    {
        s_keyBell=5;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_L,0);
    }
...

    {
        ...
        if(Hci_AbedOrAway.bit0==0)
        {
            if( ((i32>=(ModuleMemory_psPara->ThreshsholdValue2+(4300L*ModuleMemory_psPara->Offset_H[0])))&&(0==((ModuleMemory_psPara->ThreshsholdValue2)&0x80000000)))
                ||((i32>=(ModuleMemory_psPara->ThreshsholdValue2-0x80000000-(4300L*ModuleMemory_psPara->Offset_H[1])))&&(1==((ModuleMemory_psPara->ThreshsholdValue2)&0x80000000))))
            {
                Hci_OffSet_TimerS++;
            }
            else
        ...
        else
        {
            if( ((i32<=(ModuleMemory_psPara->ThreshsholdValue2+(4300*ModuleMemory_psPara->Offset_L[0])))&&(0==((ModuleMemory_psPara->ThreshsholdValue2)&0x80000000)))
                ||((i32<=(ModuleMemory_psPara->ThreshsholdValue2-0x80000000-(4300*ModuleMemory_psPara->Offset_L[1])))&&(1==((ModuleMemory_psPara->ThreshsholdValue2)&0x80000000))))
            {
                Hci_OffSet_TimerS++;
            }
            else
    ...
    if(s_count>=10)
    {
        ...
        if(Hci_light_cmt==0)
        {
            BspLed_Mode(1,BSP_LED_E_MODE_OFF);
        }
        else
        {
            BspAp3216c_Read(&i16,NULL,NULL);
            if(i16<=ModuleMemory_psPara->ThreshsholdValue3)
            {
                BspLed_Mode(1,BSP_LED_E_MODE_ON);
            }
            else
            {
                BspLed_Mode(1,BSP_LED_E_MODE_OFF);
