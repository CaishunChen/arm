...
typedef struct BSP_ESP8266_S_TX
{
    ...
    uint8_t     len;
    uint8_t     AtCmd;
    uint8_t     Flag_state  :1;  // 0-空闲1-待发送
    uint8_t     Flag_res    :7;
    uint8_t     cmt;
} BSP_ESP8266_S_TX;
...
static void BspEsp8266_SendAT(uint8_t *pstr)
{
    if(BspEsp8266_DebugControl_Enable==ON)
    {
        return;
    }
    UART_DMA_Tx(BSP_ESP8266_UARTX, pstr, strlen((char*)pstr));
    if(BspEsp8266_DebugTest_Enable==ON)
    {
        DebugOutStr((int8_t*)pstr);
    }
}
...
uint8_t BspEsp8266_Tx(uint8_t ch,uint8_t *pbuf,uint16_t len)
{
...
void BspEsp8266_100ms(void)
{
    ...
    {
        //已通过手机设置连接wifi
        if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"smartconfig connected wifi"))
        {
        }
        //已连接服务器
        else if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"CONNECT"))
        {
        }
        //数据发送成功
        else if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"SEND OK"))
        {}
        //接收到数据
        else if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"+IPD"))
        {}
    }
    //----------发送
        ...
        */
        ...
        case AT_CWMODE:
            if(BspEsp8266_s_Tx.Flag_state    ==  0)
            {
                BspEsp8266_s_Tx.Flag_state   =   1;
                BspEsp8266_s_Tx.cmt     =   0;
                BspEsp8266_s_Tx.pbuf    =   "AT+CWMODE=3\r\n";
            }
            if(BspEsp8266_s_Rx.len!=0)
            {
                // OK
        ...
        case AT_CWJAP:
            if(BspEsp8266_s_Tx.Flag_state    ==  0)
            {
                BspEsp8266_s_Tx.Flag_state   =   1;
                BspEsp8266_s_Tx.cmt     =   0;
                BspEsp8266_s_Tx.pbuf    =   "AT+CWJAP=\"xsl\",\"19820824\"\r\n";
            }
            if(BspEsp8266_s_Rx.len!=0)
            {
                // OK
                if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"WIFI GOT IP"))
                {
                    BspEsp8266_s_Tx.Flag_state   =   0;
                    BspEsp8266_s_Tx.AtCmd   =   AT_CIFSR;
                }
                // ERR
                else if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"FAIL"))
                {
                    BspEsp8266_s_Tx.AtCmd   =   AT;
                    BspEsp8266_s_Tx.Flag_state   =   0;
                }
        ...
        case AT_CIFSR:
            ...
            if(BspEsp8266_s_Rx.len!=0)
            {
                ...
                for(j=0; j<=3; j++)
                {
                    ...
                    {
                        case 0:
                            p   =   strstr((const char *)BspEsp8266_s_Rx.pbuf,"+CIFSR:APIP");
                            p1  =   BspEsp8266_s_Info.LotalIP_AP;
                            break;
                        case 1:
                            p   =   strstr((const char *)BspEsp8266_s_Rx.pbuf,"+CIFSR:APMAC");
                            p1  =   BspEsp8266_s_Info.MacAddr_AP;
                            break;
                        case 2:
                            p   =   strstr((const char *)BspEsp8266_s_Rx.pbuf,"+CIFSR:STAIP");
                            p1  =   BspEsp8266_s_Info.LotalIP_STA;
                            break;
                        case 3:
                            p   =   strstr((const char *)BspEsp8266_s_Rx.pbuf,"+CIFSR:STAMAC");
        ...
        case AT_CIPMUX:
            if(BspEsp8266_s_Tx.Flag_state    ==  0)
            {
                BspEsp8266_s_Tx.Flag_state   =   1;
                BspEsp8266_s_Tx.cmt     =   0;
                BspEsp8266_s_Tx.pbuf    =   "AT+CIPMUX=1\r\n";
            }
            if(BspEsp8266_s_Rx.len!=0)
            {
                // OK
        ...
        case AT_CIPSTART5:
            if(BspEsp8266_s_Tx.Flag_state    ==  0)
            {
                ...
                if(BspEsp8266_s_Tx.AtCmd==AT_CIPSTART1)
                {
                    ...
                    {
                        BspEsp8266_s_Tx.pbuf    =   BSP_ESP8266_CONNECT_1;
                    }
                    else
                    {
                ...
                {
                    if(BSP_ESP8266_CONNECT_2!=NULL)
                    {
                        BspEsp8266_s_Tx.pbuf    =   BSP_ESP8266_CONNECT_2;
                    }
                    else
                    {
                ...
                {
                    if(BSP_ESP8266_CONNECT_3!=NULL)
                    {
                        BspEsp8266_s_Tx.pbuf    =   BSP_ESP8266_CONNECT_3;
                    }
                    else
                    {
                ...
                {
                    if(BSP_ESP8266_CONNECT_4!=NULL)
                    {
                        BspEsp8266_s_Tx.pbuf    =   BSP_ESP8266_CONNECT_4;
                    }
                    else
                    {
                ...
                {
                    if(BSP_ESP8266_CONNECT_5!=NULL)
                    {
                        BspEsp8266_s_Tx.pbuf    =   BSP_ESP8266_CONNECT_5;
                    }
                    else
                    {
            ...
            {
                // OK
                if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"CONNECTED")\
                   ||strstr((const char *)BspEsp8266_s_Rx.pbuf,"ERROR"))
                {
                    BspEsp8266_s_Tx.Flag_state   =   0;
                    if(BspEsp8266_s_Tx.AtCmd==AT_CIPSTART1)
                    {
                    ...
                    {
                        BspEsp8266_s_ConnectBuf[5].state    =   BSP_ESP8266_E_CONNECT_STATE_CONNECTED;
                        BspEsp8266_s_Tx.AtCmd   =   AT_CIPSEND;
                    }
                }
                BspEsp8266_s_Rx.len =   0;
        ...
        case AT_CIPSEND:
            if(BspEsp8266_s_Tx.Flag_state    ==  0)
            {
                BspEsp8266_s_Tx.Flag_state   =   1;
                BspEsp8266_s_Tx.cmt     =   0;
                BspEsp8266_s_Tx.pbuf    =   "AT+CIPSEND=0,10\r\n";
            }
            if(BspEsp8266_s_Rx.len!=0)
            {
                // OK
                if(strstr((const char *)BspEsp8266_s_Rx.pbuf,">"))
                {
                    BspEsp8266_s_Tx.Flag_state   =   0;
        ...
        case AT_CIPSEND1:
            if(BspEsp8266_s_Tx.Flag_state    ==  0)
            {
                BspEsp8266_s_Tx.Flag_state   =   1;
                BspEsp8266_s_Tx.cmt     =   0;
                BspEsp8266_s_Tx.pbuf    =   "0123456789";
            }
            if(BspEsp8266_s_Rx.len!=0)
            {
                // OK
                if(strstr((const char *)BspEsp8266_s_Rx.pbuf,"SEND OK"))
                {
                    //BspEsp8266_s_Tx.state   =   0;
                    //BspEsp8266_s_Tx.AtCmd   =   AT_CIPSEND;
                }
...
void BspEsp8266_Test_100ms(void)
{
    static uint8_t frist=1;
    if(first==1)
    {
        first=0;
    }
