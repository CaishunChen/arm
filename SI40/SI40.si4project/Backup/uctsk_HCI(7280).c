...
void Hci_100ms(void)
{
    ...
    if(Hci_step==0xFF)
    {
        ...
        if(s_led_cmt==1)
        {
			...
			BspLed_Mode(2,BSP_LED_E_MODE_ON);
			BspLed_Mode(3,BSP_LED_E_MODE_ON);
			BspLed_Mode(4,BSP_LED_E_MODE_ON);
			Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_L,0);
        }
        else if(s_led_cmt>=20)
        {
			BspLed_Mode(1,BSP_LED_E_MODE_OFF);
			BspLed_Mode(2,BSP_LED_E_MODE_OFF);
			BspLed_Mode(3,BSP_LED_E_MODE_OFF);
			BspLed_Mode(4,BSP_LED_E_MODE_OFF);
			//
			BspLed_Mode(3,BSP_LED_E_MODE_PWM);
			BspLed_Mode(4,BSP_LED_E_MODE_PWM);
            Hci_step=0;
            s_led_cmt=0;
        }
        return;
    ...
    else if(Hci_cmd==2 || Hci_cmd==3 || Hci_cmd==4 || Hci_cmd==5)
    {
        ...
        {
            case 0:
				BspLed_Mode(4,BSP_LED_E_MODE_ONCE);
                if(Hci_cmd==2)
                {
                    if(s_state.bit0==0)
                    {
