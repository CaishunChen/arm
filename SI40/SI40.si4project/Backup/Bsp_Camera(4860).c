...
static void OV_WriteReg(uint8_t _ucRegAddr, uint8_t _ucRegValue)
{
    ...
    BspGpioIIC_SendByte(OV7670_SLAVE_ADDRESS);      /* 发送设备地址+写信号 */
    BspGpioIIC_WaitAck();

    BspGpioIIC_SendByte(_ucRegAddr);                /* 发送寄存器地址 */
    BspGpioIIC_WaitAck();

    BspGpioIIC_SendByte(_ucRegValue);               /* 发送寄存器数值 */
    BspGpioIIC_WaitAck();
...
static uint8_t OV_ReadReg(uint8_t _ucRegAddr)
{
    ...
    BspGpioIIC_SendByte(OV7670_SLAVE_ADDRESS);      /* 发送设备地址+写信号 */
    BspGpioIIC_WaitAck();
    BspGpioIIC_SendByte(_ucRegAddr);                /* 发送地址 */
    BspGpioIIC_WaitAck();

    BspGpioIIC_Stop();          /* 0V7670 需要插入 stop, 否则读取寄存器失败 */

    BspGpioIIC_Start();                             /* 总线开始信号 */
    BspGpioIIC_SendByte(OV7670_SLAVE_ADDRESS + 1);/* 发送设备地址+读信号 */
    BspGpioIIC_WaitAck();

    usRegValue = BspGpioIIC_ReadByte();             /* 读出高字节数据 */
    BspGpioIIC_NAck();
    BspGpioIIC_Stop();                              /* 总线停止信号 */
