...
//
          ...
          ||defined(BOOTLOADER)||defined(PROJECT_ARMFLY_V5_XSL)\
...
typedef struct MODULE_MEMORY_S_PARA
{
    ...
    // 27
    uint32_t   ThreshsholdValue2;   // 阈值2
    uint16_t   ThreshsholdValue3;   // 阈值3(iCareB-开灯照度阈值)
    uint16_t   ThreshsholdValue4;   // 阈值4(iCareB-空床值g)
    // 29
    uint16_t   Offset_H;            // 上偏移(iCareB-g)
    uint16_t   Offset_L;            // 下偏移(iCareB-g)
    // 30
...
#define  MODULE_MEMORY_EXTFLASH_BEGIN_ADDR         512*1024L
#define  MODULE_MEMORY_EXTFLASH_WEIGHT_NUM         1
#define  MODULE_MEMORY_EXTFLASH_WEIGHT_SIZE        180*1024L
