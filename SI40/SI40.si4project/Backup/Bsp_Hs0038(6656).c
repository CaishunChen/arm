...
bool	Hs0038_Rx_Sem = false;	//全局变量，接收信号标志位
IRD_T g_tIR;									//全局变量，保存收到的解码数据
uint16_t width_buf[66] = {0};	//全局变量，保存最新收到的脉冲间隔 32个数，64个间隔，加两个同步码间隔，共66个间隔
...
static	void timer_dummy_handler(nrf_timer_event_t event_type, void * p_context){}	//定时器中断函数
...
/*设置PPI*/
void Bsp_Gpio_Capture(void)
{
	uint32_t capture_task_addr;
	uint32_t gpiote_event_addr;
	nrf_ppi_channel_t	Capture_channel = CAPTURE_PPI_CHANNEL;
						 
	nrf_drv_gpiote_in_init(BSP_HS0038_PIN,&config,Bsp_Gpiote_Event_handler);	//初始化GPIO，生成event
	
	nrf_drv_ppi_channel_alloc(&Capture_channel);													//分配ppi通道
	
	capture_task_addr = nrf_drv_timer_capture_task_address_get(&Capture_Timer,NRF_TIMER_TASK_CAPTURE3);		//得到任务地址
	gpiote_event_addr = nrf_drv_gpiote_in_event_addr_get(BSP_HS0038_PIN);															//得到事件地址
	
	nrf_drv_ppi_channel_assign(Capture_channel, gpiote_event_addr, capture_task_addr);	//使用ppi将task和event联系起来
	
	nrf_drv_ppi_channel_enable(Capture_channel);																				//使能ppi
	
	nrf_drv_gpiote_in_event_enable(BSP_HS0038_PIN, 1);																									//使能event
}

/*
*********************************************************************************************************
*	函 数 名: Bsp_Hs0038_Init
*	功能说明: Hs0038初始化
*	形    参: _无
*	返 回 值: 无
*********************************************************************************************************
...
void Bsp_Hs0038_Init(void)
{
    ...
    nrf_drv_timer_init(&Capture_Timer, &timer_cfg, timer_dummy_handler);
		...
		Bsp_Gpio_Capture();		//设置task和event
	
		nrf_drv_timer_enable(&Capture_Timer);	//使能定时器
}
...
//	if ((g_tIR.Status == 0) || (g_tIR.LastCapture == 0))	//第一个边沿，或之前的数据有错误
//	{
