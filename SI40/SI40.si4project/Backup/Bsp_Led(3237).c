...
void BspLed_100ms(void)
{
    ...
    if(BspLed_DebugTest_Enable==0)
    {
        ...
        {
        	i=BSP_LED_E_LEVEL_MAX;
            while(i--)
            {
            	if(BspLed_ModeLed[i-1][si]==BSP_LED_E_MODE_NULL)
            	{
            		continue;
            	}
                switch(BspLed_ModeLed[i-1][si])
                {
                    ...
                    case BSP_LED_E_MODE_ONCE:
                        ...
                        else
                        {
                            ...
                            BspLed_ModeLed[i-1][si]  =   BSP_LED_E_MODE_NULL;
...
void BspLed_Mode(uint8_t num,BSP_LED_E_MODE mode,BSP_LED_E_LEVEL level)
{
    ...
    if(BspLed_ModeLed[level][num-1] == mode)
    {
        return;
    }
    BspLed_ModeLed[level][num-1]   =   mode;
