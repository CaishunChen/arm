...
static void BspEsp8266_SendAT(uint8_t *pstr,uint16_t len)
{
    if(BspEsp8266_DebugControl_Enable==ON)
    {
        return;
    }
    UART_DMA_Tx(BSP_ESP8266_UARTX, pstr, len);
...
uint8_t BspEsp8266_Tx(BSP_ESP8266_E_CH ch,uint8_t *pbuf,uint16_t len)
{
    ...
    sprintf((char*)(BspEsp8266_s_Tx.buf),"AT+CIPSEND=%d,%d\r\n\0",ch,len);
    BspEsp8266_s_Tx.len         =   strlen(BspEsp8266_s_Tx.buf);
    memcpy(&BspEsp8266_s_Tx.buf[25],pbuf,len);
    BspEsp8266_s_Tx.len_Data    =   len;
    return OK;
}
...
void BspEsp8266_100ms(void)
{
    ...
    if(BspEsp8266_s_Rx.len!=0)
    {
        ...
        //数据发送成功
        else if(strstr((const char *)BspEsp8266_s_Rx.buf,"SEND OK"))
        {}
        //接收到数据
    ...
    switch(BspEsp8266_s_Tx.AtCmd)
    {
        ...
        */
        ...
        case AT_CIPSTART5:
            ...
            {
                // OK
                if(strstr((const char *)BspEsp8266_s_Rx.buf,"CONNECTED")\
                   ||strstr((const char *)BspEsp8266_s_Rx.buf,"ERROR"))
                {
                    BspEsp8266_s_Tx.Flag_state   =   0;
                    if(BspEsp8266_s_Tx.AtCmd==AT_CIPSTART1)
                    {
        ...
        case AT_CIPSEND1:
            if(BspEsp8266_s_Tx.Flag_state    ==  0)
            {
                BspEsp8266_s_Tx.Flag_state   =   1;
                BspEsp8266_s_Tx.cmt     =   0;
                memcpy(&BspEsp8266_s_Tx.buf[0],&BspEsp8266_s_Tx.buf[25],BspEsp8266_s_Tx.len_Data);
                BspEsp8266_s_Tx.len     =   BspEsp8266_s_Tx.len_Data;
            }
            if(BspEsp8266_s_Rx.len!=0)
            {
                // OK
...
void BspEsp8266_Test_100ms(void)
{
    ...
    {
        scmt=0;
        BspEsp8266_Tx(BSP_ESP8266_E_CH_5,"XSL-WIFI-TEST-SEND\r\n",strlen("XSL-WIFI-TEST-SEND\r\n"));
    }
}
