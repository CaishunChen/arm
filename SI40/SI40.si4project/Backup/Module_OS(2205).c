...
void HardFault_Handler(void)
{
    int8_t *pbuf;
    pbuf=MemManager_Get(E_MEM_MANAGER_TYPE_256B);
    sprintf((char *)pbuf, "HardFault_Handler\r\n");
    MODULE_OS_DEBUG_OUT_STR(pbuf);
    MemManager_Free(E_MEM_MANAGER_TYPE_256B,pbuf);
    while (1);
}
...
void Module_OS_Info_DebugTestOnOff(uint8_t OnOff)
{
    ...
    if(OnOff==ON)
    {
        ...
        pi32=(uint32_t*)&pbuf[240];
        ps  =(TaskStatus_t*)&pbuf[256];
        pi32[0]=xPortGetFreeHeapSize();
        pi32[1]=xPortGetMinimumEverFreeHeapSize();
        sprintf((char*)pbuf,"Heap: Total-%05ld Min-%05ld Current-%05ld\r\n",configTOTAL_HEAP_SIZE,pi32[1],pi32[0]);
        MODULE_OS_DEBUG_OUT_STR(pbuf);
        ...
        */
        ...
        MODULE_OS_DEBUG_OUT_STR(pbuf1);
        */
        pbuf[100] = uxTaskGetNumberOfTasks();
		//
		//
        pbuf[100] = uxTaskGetSystemState(ps, pbuf[100], &pi32[0]);
        if(pi32[0]>0)
        {
            for(pbuf[101]=0; pbuf[101]<pbuf[100]; pbuf[101]++)
            {
                pi32[1]=(uint64_t)(ps[ pbuf[101] ].ulRunTimeCounter)*100 / pi32[0];
                if( pi32[1] > 0UL )
                {

                    sprintf((char*)pbuf,"%-24s%-6c%-6d%-8d%-8d%d%%\r\n",ps[ pbuf[101]].pcTaskName,task_state[ps[ pbuf[101] ].eCurrentState],
                            ps[ pbuf[101] ].xTaskNumber,ps[ pbuf[101]].uxCurrentPriority,
                            ps[ pbuf[101] ].usStackHighWaterMark,pi32[1]);
                }
                else
                {
                    /* 任务运行时间不足总运行时间的1%*/
                    sprintf((char*)pbuf,"%-24s%-6c%-6d%-8d%-8dt<1%%\r\n",ps[pbuf[101] ].pcTaskName,task_state[ps[ pbuf[101] ].eCurrentState],
                            ps[ pbuf[101] ].xTaskNumber,ps[ pbuf[101]].uxCurrentPriority,
                            ps[ pbuf[101] ].usStackHighWaterMark);
                }
                MODULE_OS_DEBUG_OUT_STR(pbuf);
            }
        }
