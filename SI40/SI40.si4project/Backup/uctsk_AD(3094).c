...
//
static uint8_t AD_DebugTest_Enable=0;
//------------------------------- 函数声明 --------------------------
...
void ADC_Configuration(void)
{
    ...
    for(i=0; i<5; i++)
    {
        ...
        {
            m_channel_config.config.config.ain 	=	NRF_ADC_CONFIG_INPUT_3;
        }
        else if(i==2)
        {
            m_channel_config.config.config.ain 	=   NRF_ADC_CONFIG_INPUT_4;
        }
        else if(i==3)
        {
            m_channel_config.config.config.ain 	=   NRF_ADC_CONFIG_INPUT_5;
        }
        else if(i==4)
        {
            m_channel_config.config.config.ain 	=   NRF_ADC_CONFIG_INPUT_6;
        }
        //通道使能
        nrf_drv_adc_channel_enable(&m_channel_config);
    }
    */
    m_channel_config.config.config.ain 	=   NRF_ADC_CONFIG_INPUT_2|NRF_ADC_CONFIG_INPUT_3|NRF_ADC_CONFIG_INPUT_4|NRF_ADC_CONFIG_INPUT_5|NRF_ADC_CONFIG_INPUT_6;
	nrf_drv_adc_channel_enable(&m_channel_config);
#endif
#endif
...
{
    MODULE_OS_TASK_CREATE("App Task AD",\
                          uctsk_AD,\
                          APP_TASK_AD_PRIO,\
                          App_TaskADStk,\
...
nrf_adc_value_t i16_buf[5]= {0};
uint8_t AD_ReadValue(uint8_t ch,uint16_t *pValue)
{
