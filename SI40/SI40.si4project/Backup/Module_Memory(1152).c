...
uint8_t  Module_Memory_App(uint16_t Cmd,uint8_t *pBuf,void *pPara)
{
    ...
    switch(Cmd)
    {
        case MODULE_MEMORY_APP_CMD_GLOBAL_R:
			...
			memcpy((char*)pBuf,(char*)ModuleMemory_psPara,sizeof(MODULE_MEMORY_S_PARA));
            break;
...
#if   (defined(XKAP_ICARE_B_M))
				psPara->GprsServer		  =  1;
				psPara->Flag_GprsRegister =  ON;
                psPara->Flag_NightLight   =  ON;
                psPara->ThreshsholdValue1 =  0xFFFFFFFF;
                psPara->ThreshsholdValue2 =  1000;
                psPara->ThreshsholdValue3 =  50;
                psPara->ThreshsholdValue4 =  100;
                psPara->Offset_H[0]       =  22+20;
                psPara->Offset_L[0]       =  18+20;
...
#if     (defined(MODULE_MEMORY_INFO_ADDR))					
        {
			...
			BspRtc_ReadRealTime(NULL,NULL,&(ps_info->UnixTime),NULL);
            // 直接写
...
#if     (defined(MODULE_MEMORY_INFO_ADDR))					
        {
            ...
            else if(ps_InfoFilter->Mode==2)
            {
                ...
                for(ps_info[1].UnixTime=memory_s_InfoInfo.AllNum; ps_info[1].UnixTime!=0; ps_info[1].UnixTime--)
                {
                    ...
                    // 打印到debug
                    ptm=(struct tm *)&ps_info[(256/sizeof(MODULE_MEMORY_S_INFO))/2];
                    if(ps_InfoFilter->Flag_PrintToDebug)
                    {
						*ptm = Count_Time_ConvUnixToCalendar(ps_info->UnixTime);
                        sprintf((char*)&ps_info[2],"num-%05d,time-(%04d-%02d-%02d %02d:%02d:%02d),id-%05d,Para-%05d\r\n",\
                                ps_InfoFilter->OutBufNum+1,ptm->tm_year,ptm->tm_mon+1,ptm->tm_mday,ptm->tm_hour,ptm->tm_min,ptm->tm_sec,ps_info->ErrId,ps_info->Para);
        ...
        case MODULE_MEMORY_APP_CMD_DAY_INFO_R:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Read(MODULE_MEMORY_DAY_INFO_ADDR+MODULE_MEMORY_DAY_INFO_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_INFO_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_INFO_W:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Write(MODULE_MEMORY_DAY_INFO_ADDR+MODULE_MEMORY_DAY_INFO_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_INFO_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_SLEEPLEVEL_R:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Read(MODULE_MEMORY_DAY_SLEEPLEVEL_ADDR+MODULE_MEMORY_DAY_SLEEPLEVEL_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_SLEEPLEVEL_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_SLEEPLEVEL_W:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Write(MODULE_MEMORY_DAY_SLEEPLEVEL_ADDR+MODULE_MEMORY_DAY_SLEEPLEVEL_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_SLEEPLEVEL_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_BREATH_R:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Read(MODULE_MEMORY_DAY_BREATH_ADDR+MODULE_MEMORY_DAY_BREATH_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_BREATH_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_BREATH_W:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Write(MODULE_MEMORY_DAY_BREATH_ADDR+MODULE_MEMORY_DAY_BREATH_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_BREATH_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_HEARTRATE_R:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Read(MODULE_MEMORY_DAY_HEARTRATE_ADDR+MODULE_MEMORY_DAY_HEARTRATE_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_HEARTRATE_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_HEARTRATE_W:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Write(MODULE_MEMORY_DAY_HEARTRATE_ADDR+MODULE_MEMORY_DAY_HEARTRATE_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_HEARTRATE_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_BODYMOVE_R:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Read(MODULE_MEMORY_DAY_BODYMOVE_ADDR+MODULE_MEMORY_DAY_BODYMOVE_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_BODYMOVE_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_DAY_BODYMOVE_W:
            ...
            if(*((uint8_t*)pPara)<MODULE_MEMORY_ADDR_DAY_NUM)
            {
                res = Bsp_CpuFlash_Write(MODULE_MEMORY_DAY_BODYMOVE_ADDR+MODULE_MEMORY_DAY_BODYMOVE_SIZE*(*((uint8_t*)pPara)),pBuf,MODULE_MEMORY_DAY_BODYMOVE_SIZE);
            }
            else
            {
                res=ERR;
        ...
        case MODULE_MEMORY_APP_CMD_MOVE_10MIN_R:
            ...
            for(i32=0; i32<MODULE_MEMORY_EXTFLASH_MOVE_NUM; i32++)
            {
                Bsp_ExtFlash_Read_nByte(MODULE_MEMORY_EXTFLASH_MOVE_ADDR+i32*MODULE_MEMORY_EXTFLASH_MOVE_SIZE\
                                        ,pBuf,4);
                if((((uint8_t*)pPara)[0]==pBuf[1])&&(((uint8_t*)pPara)[1]==pBuf[2])&&(((uint8_t*)pPara)[2]==pBuf[3]))
                {
                    break;
                }
            }
            // 无此日期数据
            if((i32>=MODULE_MEMORY_EXTFLASH_MOVE_NUM)||(((uint8_t*)pPara)[3]>143))
            {
                res = ERR;
                break;
            }
            // 有数据
            Bsp_ExtFlash_Read_nByte(MODULE_MEMORY_EXTFLASH_MOVE_ADDR+i32*MODULE_MEMORY_EXTFLASH_MOVE_SIZE+4+240*((uint8_t*)pPara)[3]\
                                    ,pBuf,240);
            break;
            ...
            //
        {
            ...
            for(i=0; i<MODULE_MEMORY_EXTFLASH_MOVE_NUM; i++)
            {
                ...
                {
                    // 按日期查询看是否有有效数据
                    if(buf[1]!=((uint8_t*)pPara)[0]||buf[2]!=((uint8_t*)pPara)[1]||buf[3]!=((uint8_t*)pPara)[2])
                    {
                        continue;
                    }
                }
            ...
            // 参数: 年 月 日 10分钟索引0起始(0-143)
        {
            ...
            // 容错
            if(((uint8_t*)pPara)[3]>143 && Cmd == MODULE_MEMORY_APP_CMD_MOVE_10MIN_W)
            {
                res = ERR;
                break;
            }
            ...
            if(i32>=MODULE_MEMORY_EXTFLASH_MOVE_NUM)
            {
                ...
                //写入日期
                pbuf[0]   =  0;
                pbuf[1]   =  ((uint8_t*)pPara)[0];
                pbuf[2]   =  ((uint8_t*)pPara)[1];
                pbuf[3]   =  ((uint8_t*)pPara)[2];
            ...
            if(Cmd==MODULE_MEMORY_APP_CMD_MOVE_10MIN_W)
            {
                ...
                if(i>=240)
                {
                    Bsp_ExtFlash_Write_nByte(MODULE_MEMORY_EXTFLASH_MOVE_ADDR+i32*MODULE_MEMORY_EXTFLASH_MOVE_SIZE+4+240*((uint8_t*)pPara)[3]\
                                             ,pBuf,240);
                }
            }
            ...
            {
                //查看是否为空
                Bsp_ExtFlash_Read_nByte(MODULE_MEMORY_EXTFLASH_MOVE_ADDR+i32*MODULE_MEMORY_EXTFLASH_MOVE_SIZE+4+24*(((uint8_t*)pPara)[3]*60+((uint8_t*)pPara)[4])\
                                        ,pbuf,24);
                for(i=0; i<24; i++)
                {
                    if(pbuf[i]!=0xFF)
                ...
                if(i>=24)
                {
                    Bsp_ExtFlash_Write_nByte(MODULE_MEMORY_EXTFLASH_MOVE_ADDR+i32*MODULE_MEMORY_EXTFLASH_MOVE_SIZE+4+24*(((uint8_t*)pPara)[3]*60+((uint8_t*)pPara)[4])\
                                             ,pBuf,24);
                }
            }
            //如果不为空则覆盖(暂时不做)
        ...
        case MODULE_MEMORY_APP_CMD_WEIGHT_R:
            // 读取索引
            i32=*((uint32_t*)pPara);
            if(i32>=MODULE_MEMORY_EXTFLASH_WEIGHT_NUM)
            {
                res=ERR;
                break;
            }
...
void Module_Memory_100ms(void)
{
    ...
    pbuf = MemManager_Get(E_MEM_MANAGER_TYPE_256B);
    {
        ...
        for(i32=0; i32<MODULE_MEMORY_EXTFLASH_WEIGHT_NUM; i32++)
        {
            if(memory_WeightLock==0)
            {
                break;
 ...
 * @return  None
 */
void Module_Memory_DebugTest_OnOff(uint8_t OnOff)
{
    uint8_t *pbuf;
    uint8_t i=0;
    uint32_t i32;
 ...
 * @return  None
 */
void Module_Memory_DebugTest_FactoryOnOff(uint8_t OnOff)
{
    uint8_t i;
    i = MODULE_MEMORY_CMD_FACTORY_MODE_ALL;
    Module_Memory_App(MODULE_MEMORY_APP_CMD_FACTORY,NULL,&i);
 ...
 * @return  None
 */
void Module_Memory_DebugTest_ReadParaOnOff(uint8_t OnOff)
{
    uint32_t *pbuf;
    pbuf = MemManager_Get(E_MEM_MANAGER_TYPE_256B);
	// 打印地址分配
	sprintf((char*)pbuf,"Debug Print CpuFlash Info\r\n");
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> Chip:%s\r\n",BSP_CPU_MODEL_STR);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> TotalSize:0x%08X\r\n",BSP_CPU_FLASH_SIZE);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> All     Addr Range:0x%08lX-0x%08lX\r\n",BSP_CPU_FLASH_BASE_ADDR,BSP_CPU_FLASH_END_ADDR);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> Boot    Addr Range:0x%08lX-0x%08lX\r\n",BSP_CPU_FLASH_BOOT_ADDR,BSP_CPU_FLASH_BOOT_ADDR_END);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> Para    Addr Range:0x%08lX-0x%08lX\r\n",BSP_CPU_FLASH_PARA_ADDR,BSP_CPU_FLASH_PARA_ADDR_END);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> ParaBak Addr Range:0x%08lX-0x%08lX\r\n",BSP_CPU_FLASH_PARA_BACKUP_ADDR,BSP_CPU_FLASH_PARA_BACKUP_ADDR_END);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> App     Addr Range:0x%08lX-0x%08lX\r\n",BSP_CPU_FLASH_APP_ADDR,BSP_CPU_FLASH_APP_ADDR_END);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> AppBak  Addr Range:0x%08lX-0x%08lX\r\n",BSP_CPU_FLASH_APP_BACKUP_ADDR,BSP_CPU_FLASH_APP_BACKUP_ADDR_END);
    DebugOutStr((int8_t*)pbuf);	
	//
#ifdef	MODULE_MEMORY_EXTFLASH_INIT	
	sprintf((char*)pbuf,"Debug Print ExtFlash Info\r\n");
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> Chip              :%s\r\n",BspExtFlash_s_Flash.ChipName);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> ID                :%ld\r\n",BspExtFlash_s_Flash.ChipID);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> TotalSize         :0x%08X\r\n",BspExtFlash_s_Flash.TotalSize);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> PageSize          :0x%08X\r\n",BspExtFlash_s_Flash.PageSize);
    DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"-> AddrRange         :0x%08lX-0x%08lX\r\n",0,BspExtFlash_s_Flash.TotalSize);
    DebugOutStr((int8_t*)pbuf);
#endif	
	// 打印地址分配
	sprintf((char*)pbuf,"Debug Print Addr Allocation:\r\n");
    DebugOutStr((int8_t*)pbuf);
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_1_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_1_STR,MODULE_MEMORY_CPUFLASH_ADDR_1_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_1_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_2_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_2_STR,MODULE_MEMORY_CPUFLASH_ADDR_2_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_2_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_3_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_3_STR,MODULE_MEMORY_CPUFLASH_ADDR_3_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_3_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_4_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_4_STR,MODULE_MEMORY_CPUFLASH_ADDR_4_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_4_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_5_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_5_STR,MODULE_MEMORY_CPUFLASH_ADDR_5_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_5_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_6_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_6_STR,MODULE_MEMORY_CPUFLASH_ADDR_6_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_6_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_7_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_7_STR,MODULE_MEMORY_CPUFLASH_ADDR_7_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_7_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_8_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_8_STR,MODULE_MEMORY_CPUFLASH_ADDR_8_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_8_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_9_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_9_STR,MODULE_MEMORY_CPUFLASH_ADDR_9_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_9_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_CPUFLASH_ADDR_10_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_CPUFLASH_ADDR_10_STR,MODULE_MEMORY_CPUFLASH_ADDR_10_BEGIN,MODULE_MEMORY_CPUFLASH_ADDR_10_END);
    DebugOutStr((int8_t*)pbuf);
#endif	
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_1_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_1_STR,MODULE_MEMORY_EXTFLASH_ADDR_1_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_1_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_2_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_2_STR,MODULE_MEMORY_EXTFLASH_ADDR_2_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_2_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_3_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_3_STR,MODULE_MEMORY_EXTFLASH_ADDR_3_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_3_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_4_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_4_STR,MODULE_MEMORY_EXTFLASH_ADDR_4_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_4_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_5_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_5_STR,MODULE_MEMORY_EXTFLASH_ADDR_5_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_5_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_6_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_6_STR,MODULE_MEMORY_EXTFLASH_ADDR_6_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_6_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_7_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_7_STR,MODULE_MEMORY_EXTFLASH_ADDR_7_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_7_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_8_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_8_STR,MODULE_MEMORY_EXTFLASH_ADDR_8_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_8_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_9_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_9_STR,MODULE_MEMORY_EXTFLASH_ADDR_9_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_9_END);
    DebugOutStr((int8_t*)pbuf);
#endif
#ifdef MODULE_MEMORY_EXTFLASH_ADDR_10_STR
	sprintf((char*)pbuf,"-> %-18s:0x%08lX-0x%08lX\r\n",MODULE_MEMORY_EXTFLASH_ADDR_10_STR,MODULE_MEMORY_EXTFLASH_ADDR_10_BEGIN,MODULE_MEMORY_EXTFLASH_ADDR_10_END);
    DebugOutStr((int8_t*)pbuf);
#endif
	//
