...
/* Private define ------------------------------------------------------------*/
static uint8_t Wifi_State=Idle;
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
/* Extern variables ----------------------------------------------------------*/
...
void BspEsp8266_100ms(void)
{
    ...
    switch(Wifi_State)
    {
        ...
        case AT,                    //测试启动
                 case ATE,                    //开关回显
                          case  AT_UART,                //设置UART
                                    case CIPSTATUS,              //查询网络连接状态

                                             case AT_CWMODE,              //设置工作模式
                                                      case  AT_CWAUTOCONN,          //使能上电自动连接
                                                                case  AT_CWSTARTSMART,        //使能自动配置网络

                                                                          case AT_WAITFORCONFIG,       //等待app配置完成

                                                                                   case AT_CIPSTART,            //设置udp连接
//      AT_CIPMODE,         //设置透传模式
                                                                                            case  AT_CIPSEND,             //申请发送数据
                                                                                                      case AT_SEND_SERVER_DATA,   //向服务器发送数据
                                                                                                               case  AT_WAIT_FOR_RESPOND,   //等待服务器

//      AT_RST,             //重启
//      AT_GMR,             //查询版本信息

                                                                                                                         case  AT_SEND,                //发送数据
                                                                                                                                   case  AT_RECEIVE,             //接收数据
                                                                                                                                             case  AT_RST
                                                                    }
    //----------接收数据
    //Task_Wifi_App();
    /*
    if(Bsp_Wifi_Rx_Data_Sem == 1)
    {
        Bsp_Wifi_Rx_Data_Sem = 0;
        {
            while(OK != Task_Wifi_Send("0123456789",strlen("0123456789")));
        }
    }
    */
}
