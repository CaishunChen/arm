...
{
    MODULE_OS_TASK_CREATE("App Task Hci",\
                          uctsk_Hci,\
                          APP_TASK_HCI_PRIO,\
                          App_TaskHciStk,\
...
static void Hci_Menu_SleepCurve(void)
{
    ...
    else
    {
        ...
        //标题
        res=sprintf((char*)pbuf,"第%01d条记录: 睡眠结构曲线",Sleep_cnt);
        //黄色显示"睡眠分级曲线"
        LCD_DispStr((g_LcdWidth-res*8)/2, 8, (char*)pbuf, &sfont);
        //白色重画
        sprintf((char*)pbuf,"第%01d条记录: ",Sleep_cnt);
        sfont.FrontColor=CL_WHITE;
        LCD_DispStr((g_LcdWidth-res*8)/2, 8, (char*)pbuf, &sfont);
        //"评分   分"
        sfont.FontCode=FC_ST_16;
        sfont.FrontColor=CL_CYAN;
        sprintf((char*)pbuf,"评分");
        ...
        //画曲线
        {
            ...
            */
            ...
            if(UCTSK_RFMS_E_RUNSTATE_IDLE==uctsk_Rfms_GetRunState())
            {
                ...
                sfont.FontCode=FC_ST_16;
                sfont.FrontColor=CL_WHITE;
                sprintf((char*)pbuf,"监测时间:%02d月%02d日%02d:%02d - %02d月%02d日%02d:%02d",\
        ...
        if(psDayInfo->Flag_DataValid==1)
        {
            sfont.FontCode=FC_ST_12;
            sfont.FrontColor=RGB(255,  100,  0);
            sprintf((char*)pbuf,"监测无效");
            LCD_DispStr(160, 139, (char*)pbuf, &sfont);
            sprintf((char*)pbuf,"请注意监测环境或联系客服");
        ...
        {
            //未上传
            sprintf((char*)pbuf,"(待传)");
            sfont.FrontColor   =  CL_RED;
        }
        else
        {
            //已上传
            sprintf((char*)pbuf,"(已传)");
    ...
    if(BspKey_NewSign==1)
    {
        ...
        switch(BspKey_Value)
        {
            ...
            case HCI_KEY_RIGHT:
                ...
                {
                    curve_cnt++;
                    if(curve_cnt>MODULE_MEMORY_ADDR_DAY_NUM)
                        curve_cnt = 1;
...
static void Hci_Menu_BreathHeartCurve(void)
{
    ...
    else
    {
        ...
        pbuf=MemManager_Get(E_MEM_MANAGER_TYPE_256B);
        //标题
        res=sprintf((char*)pbuf,"第%01d条记录: 呼吸心率曲线",BreathHeart_cnt);
        LCD_DispStr((g_LcdWidth-res*8)/2, 8, (char*)pbuf, &sfont);
        //重画颜色
        sprintf((char*)pbuf,"第%01d条记录: ",BreathHeart_cnt);
        sfont.FrontColor=CL_WHITE;
        LCD_DispStr((g_LcdWidth-res*8)/2, 8, (char*)pbuf, &sfont);
        //"呼 吸"，"(次/分)"
        sfont.FontCode=FC_ST_16;
        sfont.FrontColor=CL_CYAN;
        sprintf((char*)pbuf,"呼 吸");
        LCD_DispStr(5, 75, (char*)pbuf, &sfont);
        sfont.FrontColor=CL_WHITE;
        sprintf((char*)pbuf,"(次/分)");
        LCD_DispStr(0, 93, (char*)pbuf, &sfont);
        //"心 率"，"(次/分)"
        sfont.FontCode=FC_ST_16;
        sfont.FrontColor=CL_RED;
        sprintf((char*)pbuf,"心 率");
...
static void Hci_Menu_DayMoveCurve(void)
{
    ...
    else
    {
        ...
        {
            res=sprintf((char*)pbuf,"第%01d条记录: 体动记录(%02d/%02d)",Hci_DayMoveCnt+1,pbuf[252],pbuf[253]);
        ...
        {
            res=sprintf((char*)pbuf,"第%01d条记录: 体动记录(无数据)",Hci_DayMoveCnt+1);
            Hci_DayMoveDate[0]=0;
            Hci_DayMoveDate[1]=0;
            Hci_DayMoveDate[2]=0;
        }
        ...
        //白色重画
        sprintf((char*)pbuf,"第%01d条记录: ",Hci_DayMoveCnt+1);
        sfont.FrontColor=CL_WHITE;
        LCD_DispStr((g_LcdWidth-res*8)/2-16, 8, (char*)pbuf, &sfont);
        //显示show
        if(Hci_cursor==1)
...
void Hci_100ms(void)
{
    ...
    else if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]>3*100 && s_keyBell==0)
    {
        s_keyBell=1;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_B,0);
    }
    else if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]>6*100 && s_keyBell==1)
    {
        s_keyBell=2;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BB,0);
    }
    else if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]>9*100 && s_keyBell==2)
    {
        s_keyBell=3;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BBB,0);
    }
...

    {
        ...
        {
            if(i32>=(ModuleMemory_psPara->ThreshsholdValue4+ModuleMemory_psPara->Offset_H))
            {
                Hci_OffSet_TimerS++;
            }
            else
        ...
        {
            if(i32<=(ModuleMemory_psPara->ThreshsholdValue4-ModuleMemory_psPara->Offset_L))
            {
                Hci_OffSet_TimerS++;
            }
            else
        ...
        if(memory_WeightLock==1||GprsApp_Xkap_RtcReady==0)
        {
            Hci_sWeight.num=0;
        }
        else
        {
            if(Hci_sWeight.num==0)
            {
                memcpy((char*)(Hci_sWeight.DataTime),(char*)BspRtc_CurrentTimeBuf,6);
            }
            Hci_sWeight.Buf[Hci_sWeight.num] =  (uint16_t)i32;
            Hci_sWeight.num++;
            if(Hci_sWeight.num==60)
            {
                //存储
    ...
    else if(Hci_cmd==1)
    {
        switch(Hci_step)
        {
            ...
            default:
                BSP_LED_LED3_OFF;
                Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BL,0);
                Hci_step=  0;
                Hci_cmd =  0;
                break;
        }
    ...
    else if(Hci_cmd==2 || Hci_cmd==3 || Hci_cmd==4 || Hci_cmd==5)
    {
        switch(Hci_step)
        {
            case 0:
                ...
                else if(Hci_cmd==3 && s_sos_state==0)
                {
                    Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_B,0);
                }
                else if(Hci_cmd==4 && s_sos_state==0)
                {
                    Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BB,0);
                }
                else if(Hci_cmd==5 && s_sos_state==0)
                {
                    Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BBB,0);
                }
