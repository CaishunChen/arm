...
static void Hci_Menu_InfoWifi(void);
...
void  App_Hci_Init (void)
{
...
#endif
    {
        ...
        {
#if   ((HARDWARE_VER==1) || (HARDWARE_VER==7) || (HARDWARE_VER==8) || (HARDWARE_VER==9)|| (HARDWARE_VER==10)|| (HARDWARE_VER==11)||(HARDWARE_VER==0xF1))
            {
...
static void Hci_Menu_Default1(void)
{
    ...
    else
    {
        ...
        else if(Hci_pbuf[17]==1)
        {
            ...
            Hci_cursor|=(1<<1);
            //
#if   ((HARDWARE_VER==1) || (HARDWARE_VER==10)|| (HARDWARE_VER==11))
            {
                extern GUI_CONST_STORAGE GUI_BITMAP bmres36;
                GUI_DrawBitmap(&bmres36,0,0);
        ...
        else if(Hci_pbuf[17]==3)
        {
            ...
            //判断自动监测
            //当前版本关闭自动监测提示窗口
            res = ModuleMemory_psPara->RtcAutoOnOff;
            res = OFF;
            //启动自动监测
            if(ON==res)
            {
        ...
        switch(BspKey_Value)
        {
            ...
            case HCI_KEY_RIGHT:
                if(BspKey_KeepTimer_ms<2000)
                {
                    ...
                    BspRtc_ReadRealTime(NULL,ptm,NULL,NULL);
#if   ((HARDWARE_VER==7)||(HARDWARE_VER==10)|| (HARDWARE_VER==11))
...
static void Hci_Menu_II_Info(void)
{
    ...
    else
    {
        ...
        LCD_DispStr(xpos, ypos+=20, (char*)pbuf, &sfont);
        sprintf((char*)pbuf," 10.Info Wifi Module");
        LCD_DispStr(xpos, ypos+=20, (char*)pbuf, &sfont);
        //显示光标
        LCD_DispStr(xpos, (Hci_cursor+1)*20, "*", &sfont);
        //释放缓存
            ...
            case HCI_KEY_ENTER:
                switch(Hci_cursor)
                {
                    ...
                    case 8:
                        ...
                        break;
                    case 9:
                        pHci_MenuFun = Hci_Menu_InfoWifi;
                        break;
                    default:
                        break;
                }
...
static void Hci_Menu_SetTime(void)
{
    ...
    else
    {
        ...
        if(Hci_ParaChangeSign==0)
        {
            ...
            //自启动
            Hci_pbuf[15] = ModuleMemory_psPara->RtcAutoOnOff;
            Hci_pbuf[16] =  (ModuleMemory_psPara->RtcAutoBeginTime)>>8;
            Hci_pbuf[17] =  (ModuleMemory_psPara->RtcAutoBeginTime)&0x00FF;
            Hci_pbuf[18] =  (ModuleMemory_psPara->RtcAutoEndTime)>>8;
            Hci_pbuf[19] =  (ModuleMemory_psPara->RtcAutoEndTime)&0x00FF;
            //时间格式
...
static void Hci_Menu_InfoVer(void)
{
    ...
    else
    {
        ...
        LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
        //显示硬件版本
        sprintf((char*)pbuf,"Hardware Ver(Hex): %02x.%02x",HARDWARE_VER,HARDWARE_SUB_VER);
        LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
        //显示软件版本
        sprintf((char*)pbuf,"Software Ver(Hex): %02x.%02x",SOFTWARE_VER,SOFTWARE_SUB_VER);
        LCD_DispStr(0, ypos+=20, (char*)pbuf , &sfont);
        //显示出厂日期
...
static void Hci_Menu_InfoWifi(void)
{
    ...
    else
    {
        Hci_ReShowTimer=10;
        //
        pbuf=(uint8_t*)"Menu-Info WifiModule";
        LCD_DispStr((g_LcdWidth-strlen((char*)pbuf)*8)/2, 0,(char*)pbuf, &sfont);
        //申请缓存
        pbuf=MemManager_Get(E_MEM_MANAGER_TYPE_256B);
        if(BspEsp8266_s_Info.Flag_Install==0)
        {
            sprintf((char*)pbuf,"Not found the Wifi module!");
            LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
        }
        else
        {
            sprintf((char*)pbuf,"OnOff-%d Signal:%02d Mode:%d",BspEsp8266_s_Info.Flag_OnOff,BspEsp8266_s_Info.SignalVal,BspEsp8266_s_Info.Flag_WordMode);
            LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
            sprintf((char*)pbuf,"LotalIp AP :%03d.%03d.%03d.%03d",BspEsp8266_s_Info.LotalIP_AP[0],BspEsp8266_s_Info.LotalIP_AP[1],BspEsp8266_s_Info.LotalIP_AP[2],BspEsp8266_s_Info.LotalIP_AP[3]);
            LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
            sprintf((char*)pbuf,"LotalIp STA:%03d.%03d.%03d.%03d",BspEsp8266_s_Info.LotalIP_STA[0],BspEsp8266_s_Info.LotalIP_STA[1],BspEsp8266_s_Info.LotalIP_STA[2],BspEsp8266_s_Info.LotalIP_STA[3]);
            LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
            sprintf((char*)pbuf,"MacAddr AP :%02X:%02X:%02X:%02X:%02X:%02X",\
                    BspEsp8266_s_Info.MacAddr_AP[0],BspEsp8266_s_Info.MacAddr_AP[1],BspEsp8266_s_Info.MacAddr_AP[2],\
                    BspEsp8266_s_Info.MacAddr_AP[3],BspEsp8266_s_Info.MacAddr_AP[4],BspEsp8266_s_Info.MacAddr_AP[5]);
            LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
            sprintf((char*)pbuf,"MacAddr STA:%02X:%02X:%02X:%02X:%02X:%02X",\
                    BspEsp8266_s_Info.MacAddr_STA[0],BspEsp8266_s_Info.MacAddr_STA[1],BspEsp8266_s_Info.MacAddr_STA[2],\
                    BspEsp8266_s_Info.MacAddr_STA[3],BspEsp8266_s_Info.MacAddr_STA[4],BspEsp8266_s_Info.MacAddr_STA[5]);
            LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
            for(pbuf[100]=0; pbuf[100]<BSP_ESP8266_E_CH_MAX; pbuf[100]++)
            {
                sprintf((char*)pbuf,"CH%d: %d Tx=%09d Rx=%09d",pbuf[100]+1,BspEsp8266_s_Info.ConnectStateBuf[pbuf[100]],BspEsp8266_s_Info.TxNumBuf[pbuf[100]],BspEsp8266_s_Info.RxNumBuf[pbuf[100]]);
                LCD_DispStr(0, ypos+=20, (char*)pbuf, &sfont);
            }
            //
        }
    ...
    if(BspKey_NewSign==1)
    {
        ...
        {
            case HCI_KEY_ENTER:
                BspEsp8266_WorkMode(BSP_ESP8266_E_WORKMODE_SET);
                break;
            case HCI_KEY_ESC:
                pHci_MenuFun = Hci_Menu_II_Info;
                //
...
static void Hci_Menu_SetMenu(void)
{
    ...
    if(Hci_NoFirst   == 0)
    {
        ...
        //读取定时测量信息
        Hci_pbuf[11]=ModuleMemory_psPara->RtcAutoOnOff;
        Hci_pbuf[12]=(ModuleMemory_psPara->RtcAutoBeginTime)>>8;
        Hci_pbuf[13]=(ModuleMemory_psPara->RtcAutoBeginTime);
        Hci_pbuf[14]=(ModuleMemory_psPara->RtcAutoEndTime)>>8;
        Hci_pbuf[15]=(ModuleMemory_psPara->RtcAutoEndTime);
        //读取入睡提醒信息
...
static void Hci_Menu_SleepCurve(void)
{
    ...
    {
        Hci_ReShowTimer=0xFFFF;
#if ((HARDWARE_VER==10)|| (HARDWARE_VER==11))
        // 画背景
#if     (HARDWARE_VER==10)
        {
            extern GUI_CONST_STORAGE GUI_BITMAP bmres37;
            GUI_DrawBitmap(&bmres37,0,0);
        }
#elif   (HARDWARE_VER==11)
        {
            extern GUI_CONST_STORAGE GUI_BITMAP bmres38;
            GUI_DrawBitmap(&bmres38,0,0);
        }
#endif
        // 申请缓存
        pbuf            =  MemManager_Get(E_MEM_MANAGER_TYPE_256B);
        ...
        //-----------------
        if((Hci_SleepDataValid(psDaySleepLevel->DataBuf, sizeof(psDaySleepLevel->DataBuf)) == ERR)/*||psDayInfo->Flag_DataValid==1*/)
        {
            GUI_SetColor(GUI_RED);
            GUI_SetPenSize(8);
            GUI_DrawCircle(320/2,240/2,40);
            GUI_DrawCircle(320/2,240/2,39);
            GUI_DrawCircle(320/2,240/2,38);
            GUI_DrawCircle(320/2,240/2,37);
            GUI_DrawCircle(320/2,240/2,36);
            GUI_DrawCircle(320/2,240/2,35);
            GUI_DrawCircle(320/2,240/2,34);
            GUI_DrawCircle(320/2,240/2,33);
            GUI_DrawLine(320/2-26,240/2-26,320/2+26,240/2+26);
        }
        else
        {

            // 标题
            res=sprintf((char*)pbuf,pSTR_SLEEP_CURVE_STR2[Main_Language],Sleep_cnt);
            sfont.FrontColor=CL_WHITE;
            ...
            // 分数
            GUI_SetFont(&GUI_FontFont_HNLP_72);
            GUI_SetTextMode(GUI_TM_XOR|GUI_TM_TRANS);
            sprintf((char*)pbuf,"%02d",psDayInfo->Scoring);
            GUI_DispStringHCenterAt((char*)pbuf,320/2,240/2-32);
            //标记上传状态
...
#elif   ((HARDWARE_VER==10)|| (HARDWARE_VER==11))
                    curve_cnt--;
                    if(curve_cnt == 0)
                        curve_cnt = MODULE_MEMORY_ADDR_DAY_NUM*2-1;
#else
...
#define BOX1_TEXT   "Camera 0V7670 Test"
...
static HCI_S_WEIGHT Hci_s_Weight30s;
//
...
void Hci_100ms(void)
{
    ...
    if(Hci_step==0xFF)
    {
...
#endif
            //
            nrf_gpio_cfg_input(HCI_SW1_PIN, NRF_GPIO_PIN_PULLUP);
            nrf_gpio_cfg_input(HCI_SW2_PIN, NRF_GPIO_PIN_PULLUP);
            //
        ...
        //
        memset((char*)&Hci_s_Weight30s,0,sizeof(Hci_s_Weight30s));
        return;
    }
#if (HARDWARE_SUB_VER==3)
    //网络指示灯(未联网1s，联网3s，配网0.5s)
    if (1)
    {
        static char si = 0;
        si++;
        ...
        //
        if (BspEsp8266_s_Info.Flag_WordMode==BSP_ESP8266_E_WORKMODE_NORMAL && BspEsp8266_s_Info.Flag_ConnectAP==1)
        {
            if (si == 1)
            {
                BSP_LED_LED5_ON;
        ...
        else if (BspEsp8266_s_Info.Flag_WordMode==BSP_ESP8266_E_WORKMODE_NORMAL && BspEsp8266_s_Info.Flag_ConnectAP==0)
        {
            if (si == 1)
            {
                BSP_LED_LED5_ON;
        ...
        else if (BspEsp8266_s_Info.Flag_WordMode==BSP_ESP8266_E_WORKMODE_SET)
        {
            if (si == 1)
            {
                BSP_LED_LED5_ON;
...
#endif
    //校准
    if(ModuleMemory_psPara->ThreshsholdValue2==0xFFFFFFFF)
    {
        MODULE_MEMORY_S_PARA *pPara;
    ...
    //
    if(GprsApp_RxLastToNowS>180)
    {
        BspLed_Mode(1,BSP_LED_E_MODE_OFF,BSP_LED_E_LEVEL_4);
        BspLed_Mode(2,BSP_LED_E_MODE_ON,BSP_LED_E_LEVEL_4);
        BspLed_Mode(3,BSP_LED_E_MODE_OFF,BSP_LED_E_LEVEL_4);
    ...
    if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]==0xFFFF || BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]==0)
    {
        ...
        switch(s_keyBell)
        {
            ...
            case 5:
                ...
                {
                    // 恢复出厂设置(重启)
                    Module_Memory_DebugTest_FactoryOnOff(1);
                }
                break;
            default:
                break;
    ...
    {
        s_keyBell=5;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_BBBBB,0);
    }
    else if(BspKey_PressKeepTimerBuf_10ms[HCI_KEY_ENTER-1]>11*100 && s_keyBell==5)
    {
        s_keyBell=6;
        Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_L,0);
    }
    ...
    if(BspKey_NewSign==1)
    {
        ...
        switch(BspKey_Value)
        {
            ...
            case HCI_KEY_SOS_4:
                ...
                break;
            case HCI_KEY_SOS_2A4:
                if(Hci_cmd==0)
                {
                    Hci_cmd=9;
                }
                DebugOutStr((int8_t*)"KEY-SOS-2A4\r\n");
                break;
#if   (defined(HAIER))
            case HCI_KEY_ENTER:
                // 设备校准
    ...
    if(s_count>=10)
    {
        s_count=0;
        //
        while(Hci_s_Weight30s.Flag_Lock==1)
        {
            MODULE_OS_DELAY_MS(1);
        }
        Hci_s_Weight30s.Flag_Lock=1;
        //
        Count_Sort_Buble_U32(s32_buf,10);
        s32_buf[0]=0;
        for(i16=2; i16<=7; i16++)
        {
            s32_buf[0]  +=  s32_buf[i16];
        }
        s32_buf[0]  =   s32_buf[0]/6;
        //
        Hci_s_Weight30s.Hx711[Hci_s_Weight30s.Num++]=s32_buf[0];
        if(Hci_s_Weight30s.Num>=30)
        {
            Hci_s_Weight30s.Num=0;
        }
        Hci_s_Weight30s.Flag_Lock=0;
        //AD调试
        //
        /*
        ...
        */
        ...
        {
            if((BspAp3216c_Als<=ModuleMemory_psPara->ThreshsholdValue3) && (ModuleMemory_psPara->Flag_NightLight==ON))
            {
                BspLed_Mode(1,BSP_LED_E_MODE_ON,BSP_LED_E_LEVEL_LOW);
            }
            else if(BspAp3216c_Als>(ModuleMemory_psPara->ThreshsholdValue4))
            {
                BspLed_Mode(1,BSP_LED_E_MODE_OFF,BSP_LED_E_LEVEL_LOW);
            }
        }
...
#if (SOFTWARE_VER==1)
    {
        static uint16_t scmt=0;
        if(Hci_AbedOrAway.bit0==0)
        {
            nrf_gpio_pin_write(18,0);
            if(Hci_AbedOrAway.bit2==1)
            {
                Hci_AbedOrAway.bit2=0;
                scmt=0;
            }
            scmt++;
            if(scmt==1)
        ...
        {
            nrf_gpio_pin_write(17,0);
            if(Hci_AbedOrAway.bit2==0)
            {
                Hci_AbedOrAway.bit2=1;
                scmt=0;
            }
            scmt++;
            if(scmt==1)
    ...
    else if(Hci_cmd==1 || Hci_cmd==8)
    {
        switch(Hci_step)
        {
            case 0:
            {
                ...
                BspHx711_Get_ad(&i32,10);
                pPara   =   MemManager_Get(E_MEM_MANAGER_TYPE_256B);
                ps_info =   (MODULE_MEMORY_S_INFO*)&pPara[1];
                ps_info->Para       =   (uint16_t)(i32>>8);
                Module_Memory_App(MODULE_MEMORY_APP_CMD_GLOBAL_R,(uint8_t*)pPara,NULL);
                if(Hci_cmd==1)
                {
                    pPara->ThreshsholdValue2=i32;
                    ps_info->ErrId      =   MODULE_E_INFO_ICAREB_MANUAL_CAL_NULL;
                }
                else
                {
                    pPara->ThreshsholdValue2=i32|0x80000000;
                    ps_info->ErrId      =   MODULE_E_INFO_ICAREB_MANUAL_CAL_FULL;
                }
                Module_Memory_App(MODULE_MEMORY_APP_CMD_GLOBAL_W,(uint8_t*)pPara,NULL);
    ...
    // SOS鎸夐敭浜嬩欢
    else if(Hci_cmd==2 || Hci_cmd==3 || Hci_cmd==4 || Hci_cmd==5 || Hci_cmd==9)
    {
        switch(Hci_step)
        {
            case 0:
                ...
                else if(Hci_cmd==9 && s_state.bit0==0)
                {
                    Bsp_BuzzerMusic_PlayStop(ON,BSP_BUZZER_MUSIC_LIB_B,0);
                    BspLed_Mode(4,BSP_LED_E_MODE_ONCE,BSP_LED_E_LEVEL_2);
                }
                ...
                if(GprsAppXkap_S_Info.state!=1)
                {
                    ...
                    GprsAppXkap_S_Info.Value[0]=0;
                    */
                    GprsAppXkap_S_Info.Tag=5;
                    GprsAppXkap_S_Info.Value[0]=Hci_cmd;
                    memcpy(GprsAppXkap_S_Info.DateTime,BspRtc_CurrentTimeBuf,6);
                }
                Hci_timer=0;
                Hci_step++;
...
void Hci_GetWeight30s(uint8_t* pbuf)
{
    uint8_t i=0,j=0;
    while(Hci_s_Weight30s.Flag_Lock==1)
    {
    ...
    Hci_s_Weight30s.Flag_Lock=1;
    j=Hci_s_Weight30s.Num;
    for(i=0; i<30; i++)
    {
        pbuf[i*4+0]=Hci_s_Weight30s.Hx711[j]>>24;
        pbuf[i*4+1]=Hci_s_Weight30s.Hx711[j]>>16;
        pbuf[i*4+2]=Hci_s_Weight30s.Hx711[j]>>8;
        pbuf[i*4+3]=Hci_s_Weight30s.Hx711[j];
        j++;
        if(j>=30)
        {
            j=0;
    ...
    pbuf[i*4+0] = Hci_AbedOrAway.bit0;
    Hci_s_Weight30s.Flag_Lock=0;
}
void Hci_GetSW(uint8_t* pbuf)
{
    pbuf[0]=nrf_gpio_pin_read(HCI_SW1_PIN);
    pbuf[1]=nrf_gpio_pin_read(HCI_SW2_PIN);
}
