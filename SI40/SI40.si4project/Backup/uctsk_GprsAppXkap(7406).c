...
static void Xkap_IsleepData1(uint8_t index)
{
    ...
    // -----
    //评分->睡眠结构得分
    pbuf[i16++]   = psDayInfo->Mark_SleepStruct;
    //评分->总睡时长系数
    pbuf[i16++]   = psDayInfo->Mart_TotalSleep>>8;
    pbuf[i16++]   = psDayInfo->Mart_TotalSleep;
    //评分->入睡时长得分
    pbuf[i16++]   = psDayInfo->Mart_SleepIntoPeriod;
    //评分->体动次数得分
    pbuf[i16++]   = psDayInfo->Mark_BodyMovement;
    //评分->唤醒状态得分
    pbuf[i16++]   = psDayInfo->Mark_WakeState;
    //评分->睡眠习惯得分
    pbuf[i16++]   = psDayInfo->Mark_BeginSleepTimeChange;
    //评分->温度得分
    pbuf[i16++]   = psDayInfo->Mark_Temp;
    //评分->湿度得分
    pbuf[i16++]   = psDayInfo->Mark_Humi;
    // -----
    //赋值结束字节
