...
uint8_t  Module_Memory_App(uint16_t Cmd,uint8_t *pBuf,uint8_t *pPara)
{
    ...
    switch(Cmd)
    {
        ...
        case MODULE_MEMORY_APP_CMD_EXTFLASH_READ:
            // uint16_t Cmd,uint8_t *pBuf,uint8_t *pPara
        {
    ...
    memory_WeightLock=1;
    {
            ...
            //for(i32=0; i32<10; i32++)
        {
            ...
            for(i=0; i<60; i++)
            {
                if(i==0)
                {
                    sprintf((char*)&pbuf[128],"DateTime: %04d-%02d-%02d %02d:%02d:%02d %05d\r\n"\
                            ,2000+ps->DataTime[0],ps->DataTime[1],ps->DataTime[2],ps->DataTime[3],ps->DataTime[4],ps->DataTime[5],ps->Buf[i]);
                    DebugOutStr((int8_t*)&pbuf[128]);
                }
                else
                {
                    sprintf((char*)&pbuf[128],"                              %05d\r\n",ps->Buf[i]);
                    DebugOutStr((int8_t*)&pbuf[128]);
                }
