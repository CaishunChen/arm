...
void Module_Memory_DebugTestOnOff(uint8_t OnOff)
{
    ...
    memory_WeightLock=1;
    {
        ...
        //
        sprintf((char*)pbuf,"BeginAddr: %ld(kB)\r\n",MODULE_MEMORY_EXTFLASH_WEIGHT_ADDR/1024);
        ...
        for(i32=0; i32<MODULE_MEMORY_EXTFLASH_WEIGHT_SIZE; i32++)
        {
            ...
            for(i=0; i<10; i++)
            {
                ...
                if(i==0)
                {
                    sprintf((char*)&pbuf[128],"DateTime: %04d-%02d-%02d %02d:%02d:%02d %05d %05d %05d %05d %ld\r\n"\
                            ,2000+ps->DateTime[0],ps->DateTime[1],ps->DateTime[2],ps->DateTime[3],ps->DateTime[4],ps->DateTime[5]\
                            ,ps->AdValue[0][i],ps->AdValue[1][i],ps->AdValue[2][i],ps->AdValue[3][i],ps->Hx711[i]);
                    DebugOutStr((int8_t*)&pbuf[128]);
                }
                else
