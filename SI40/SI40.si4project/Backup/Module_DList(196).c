...
#define MODULE_DLIST_LIST_MEM_SIZE          sizeof(Node)    //  链表每单元去占用空间
 ...
 * @return  OK ERR
 */
uint8_t ModuleDList_Get(Node* pNode)
{
    // 如果 ModuleDList_ListMem_Num 未达到最大，则向后取空间
    if(ModuleDList_ListMem_EndNum < MODULE_DLIST_NODE_NUM_MAX)
    {
        pNode->pSelfNode = (Node*)(MODULE_DLIST_LIST_MEM_ADDR_BEGIN +   ModuleDList_ListMem_EndNum*sizeof(Node));
...
uint8_t ModuleDList_Save(Node* pNode)
{
    MODULE_MEMORY_S_EXTFLASH *ps_extFlash;
