...
static void Xkap_SendPara(uint8_t cmd)
{
    ...
    pbuf[i16++]   =  pPara->ThreshsholdValue2>>24;
    //---基于空床校准的在床判断偏移量
    pbuf[i16++]   =  pPara->Offset_H[0];
    //---基于空床校准的离床判断偏移量
    pbuf[i16++]   =  pPara->Offset_L[0];
    //---基于在床校准的在床判断偏移量
    pbuf[i16++]   =  pPara->Offset_H[1];
    //---基于在床校准的离床判断偏移量
    pbuf[i16++]   =  pPara->Offset_L[1];
    //---夜灯响应采集时长-100ms
    pbuf[i16++]   =  pPara->Offset_TimerS;
    //---夜灯响应时长-1s
    pbuf[i16++]   =  pPara->Reponse_TimerS;
    pbuf[i16++]   =  pPara->Reponse_TimerS>>8;
    //---照明灯功能开关(0-关闭 1-开启)
    pbuf[i16++]   =  pPara->Flag_NightLight;
    //---呼叫器连接状态(0-断开,1-连接)
    pbuf[i16++]   =  0;
    //---故障位图
    pbuf[i16++]   =  0;
    ...
    switch(pBuf[0])
    {
        ...
        case GPRSAPP_XKAP_E_CMD_PARA_W_1:
...

            {
                ...
                else if(Xkap_ParaAddr==0x0016 && Xkap_ParaLen==8))
                {
                }
                if(Xkap_ParaRes==1)
            {
        ...
        case GPRSAPP_XKAP_E_CMD_SEND_INFO:
            ...
            else
            {
                ...
                //---在床偏移量-离床---------1B
                //pBuf[29]
                if((pBuf[29]!=0) && (pBuf[29]!=0xFF) && (pBuf[29]!=pPara->Offset_H[1]))
                {
                    pPara->Offset_H[1]=pBuf[29];
                    crc1=1;
                }
                //---夜灯响应采集时长(100ms)-1B
                //pBuf[30]
