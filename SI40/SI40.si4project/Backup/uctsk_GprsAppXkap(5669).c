...
static void uctsk_GprsAppXkap (void *pvParameters)
{
    ...
    //网络连接
    {
        GprsAppXkap_Sign_Login=ModuleMemory_psPara->Flag_GprsRegister;
        if(ModuleMemory_psPara->GprsServer==0)
        {
        ...
        else if(ModuleMemory_psPara->GprsServer==1)
        {
        ...
        else if(ModuleMemory_psPara->GprsServer==2)
        {
    ...
    //等待网络连接
#if     (defined(GPRSAPP_XKAP_NET_WIFI))
    while(BspEsp8266_s_Info.ConnectStateBuf[GPRSNETAPP_XKAP_CONNECT_CH]==0)
    {
        MODULE_OS_DELAY_MS(1000);
    }

#elif   (defined(GPRSAPP_XKAP_NET_GPRS))
    ...
    for(;;)
    {
        ...
        else if(err == MODULE_OS_ERR_TIMEOUT)
        {
            ...
            //参数上传---定时上传
                ...
                //else if(((BspRtc_CurrentTimeBuf[5]==0)||(BspRtc_CurrentTimeBuf[5]==30)||s_first_up.bit0==0) && GprsApp_Xkap_RtcReady==1 && GprsAppXkap_Sign_ParaUpload==1 && GprsAppXkap_UpdataState==0)
#else
...
static void Xkap_IsleepData1(uint8_t index)
{
    ...
    // -----
    //评分->睡眠结构得分
    pbuf[i16++]   = WEIGHTSLEEPSTRCT-psDayInfo->Mark_SleepStruct;
    //评分->总睡时长系数
    pbuf[i16++]   = psDayInfo->Mart_TotalSleep>>8;
    pbuf[i16++]   = psDayInfo->Mart_TotalSleep;
    //评分->入睡时长得分
    pbuf[i16++]   = WEIGHTSLEEPINTOPERIOD-psDayInfo->Mart_SleepIntoPeriod;
    //评分->体动次数得分
    pbuf[i16++]   = WEIGHTBODYMOVEMENTCOUNT-psDayInfo->Mark_BodyMovement;
    //评分->唤醒状态得分
    pbuf[i16++]   = WEIGHTAWAKESTATE-psDayInfo->Mark_WakeState;
    //评分->睡眠习惯得分
    pbuf[i16++]   = WEIGHTSTARTTIMEDIS-psDayInfo->Mark_BeginSleepTimeChange;
    //评分->温度得分
    pbuf[i16++]   = WEIGHTSLEEPTEMP-psDayInfo->Mark_Temp;
    //评分->湿度得分
    pbuf[i16++]   = WEIGHTSLEEPHUM-psDayInfo->Mark_Humi;
    // -----
    //赋值结束字节
    pbuf[i16++]   =  psDayInfo->EndByte;
    //补齐100个字节
    memset((char*)&pbuf[i16],0,100-83-9);
    i16+=100-83-9;
    //无效数据处理
...
static void Xkap_SendPara(uint8_t cmd)
{
    ...
    uint16_t i16,crc16;
    //
    if(GprsNet_s_Info.state==1)
    {
        if(Connect_s[GPRSNETAPP_XKAP_CONNECT_CH].state!=GPRSNET_E_CONNECT_STATE_CONNECTED)return;
    }
    /*
    else if(BspEsp8266_s_Info.Flag_Install==1)
    {
    if(BspEsp8266_s_Info.ConnectStateBuf[GPRSNETAPP_XKAP_CONNECT_CH]==0)return;
    }
    */
    else
    {
    ...
    //
    //---开夜灯照度阈值
    pbuf[i16++]   =  ModuleMemory_psPara->ThreshsholdValue3;
    //---关夜灯照度阈值
    pbuf[i16++]   =  ModuleMemory_psPara->ThreshsholdValue4;
    //---空床/在床校准值
    pbuf[i16++]   =  ModuleMemory_psPara->ThreshsholdValue2;
    pbuf[i16++]   =  ModuleMemory_psPara->ThreshsholdValue2>>8;
    pbuf[i16++]   =  ModuleMemory_psPara->ThreshsholdValue2>>16;
    ...
    pbuf[i16++]   =  0;
    pbuf[i16++]   =  0;
    //---前30s的重量原始数据120B+1B在离床
    Hci_GetWeight30s(&pbuf[i16]);
    i16+=121;
    //---当前照度
    pbuf[i16++]   =  BspAp3216c_Als;
    pbuf[i16++]   =  BspAp3216c_Als>>8;
    //
    Hci_GetSW(&pbuf[i16]);
    i16+=2;
#endif
...
static void Xkap_SendInfo(GPRSAPP_XKAP_SEND_INFO *psInfo)
{
    ...
    //
#if     (defined(GPRSAPP_XKAP_NET_WIFI))
    while(ERR==BspEsp8266_Tx(GPRSNETAPP_XKAP_CONNECT_CH,pbuf,i16))
    {
        MODULE_OS_DELAY_MS(10);
    }
#elif   (defined(GPRSAPP_XKAP_NET_GPRS))
    ...
    switch(pBuf[0])
    {
        ...
        case GPRSAPP_XKAP_E_CMD_SEND_INFO:
            ...
            else
            {
                ...
                Count_TimeTypeConvert(4,ptm2,&pBuf[i]);
                if(10<abs(Count_TimeCompare(ptm1,ptm2)))
                {
                    BspRtc_SetRealTime(NULL,NULL,NULL,&pBuf[i]);
                }
                i+=6;
                //---
                crc1=0;
                ...
                //---开灯夜灯照度阈值--------1B
                //pBuf[22]
                if((pBuf[i]!=0) && (pBuf[i]!=0xFF) && (pBuf[i]!=pPara->ThreshsholdValue3))
                {
                    pPara->ThreshsholdValue3=pBuf[i];
                    crc1=1;
                }
                i++;
                //---关灯夜灯照度阈值--------1B
                //pBuf[23]
                if((pBuf[i]!=0) && (pBuf[i]!=0xFF) && (pBuf[i]!=pPara->ThreshsholdValue4))
                {
                    pPara->ThreshsholdValue4=pBuf[i];
                    crc1=1;
                }
                i++;
                //---空床(在床)校准值--------4B
                //pBuf[22]  -   pBuf[25]
                i32=Count_4ByteToLong(pBuf[i+3], pBuf[i+2], pBuf[i+1], pBuf[i]);
                ...
                //---空床偏移量-在床---------1B
                //pBuf[26]
                if((pBuf[i]!=0) && (pBuf[i]!=0xFF) && (pBuf[i]!=pPara->Offset_H[0]))
                {
                    pPara->Offset_H[0]=pBuf[i];
                    crc1=1;
                }
                i++;
                //---空床偏移量-离床---------1B
                //pBuf[27]
                if((pBuf[i]!=0) && (pBuf[i]!=0xFF) && (pBuf[i]!=pPara->Offset_L[0]))
                {
                    pPara->Offset_L[0]=pBuf[i];
                    crc1=1;
                }
                i++;
                //---在床偏移量-在床---------1B
                //pBuf[28]
                if((pBuf[i]!=0) && (pBuf[i]!=0xFF) && (pBuf[i]!=pPara->Offset_H[1]))
                {
                    pPara->Offset_H[1]=pBuf[i];
                    crc1=1;
                }
                i++;
                //---在床偏移量-离床---------1B
                //pBuf[29]
                if((pBuf[i]!=0) && (pBuf[i]!=0xFF) && (pBuf[i]!=pPara->Offset_L[1]))
                {
                    pPara->Offset_L[1]=pBuf[i];
                    crc1=1;
                }
                i++;
                //---夜灯响应采集时长(100ms)-1B
                //pBuf[30]
                if((pBuf[i]!=0) && (pBuf[i]!=0xFF) && (pBuf[i]!=pPara->Offset_TimerS))
                {
                    pPara->Offset_TimerS=pBuf[i];
                    crc1=1;
                }
                i++;
                //---夜灯响应时长(秒)--------2B
                //pBuf[31]  -   pBuf[32]
                crc2=Count_2ByteToWord(pBuf[i+1], pBuf[i]);
        ...
        case GPRSAPP_XKAP_E_CMD_PARA_W_1:
            ...
            if(1)
            {
                ...
                else if(Xkap_ParaAddr==0x0200 && Xkap_ParaLen==20)
                {
                    //---时钟--------------------6B
                    i=3;
                    //pBuf[16]  -   pBuf[21]
                    ...
                    ptm2 = &ptm1[1];
                    BspRtc_ReadRealTime(NULL,ptm1,NULL,NULL);
                    Count_TimeTypeConvert(4,ptm2,&pbuf1[i]);
                    if(10<abs(Count_TimeCompare(ptm1,ptm2)))
                    {
                        BspRtc_SetRealTime(NULL,NULL,NULL,&pbuf1[i]);
                    }
                    i+=6;
                    //---
                    Module_Memory_App(MODULE_MEMORY_APP_CMD_GLOBAL_R,(uint8_t*)pPara,NULL);
                    //---开灯夜灯照度阈值--------1B
                    //pBuf[22]
                    if((pbuf1[i]!=0) && (pbuf1[i]!=0xFF) && (pbuf1[i]!=pPara->ThreshsholdValue3))
                    {
                        pPara->ThreshsholdValue3=pbuf1[i];
                        Xkap_ParaRes    =   1;
                    }
                    i++;
                    //---关灯夜灯照度阈值--------1B
                    //pBuf[23]
                    if((pbuf1[i]!=0) && (pbuf1[i]!=0xFF) && (pbuf1[i]!=pPara->ThreshsholdValue4))
                    {
                        pPara->ThreshsholdValue4=pbuf1[i];
                        Xkap_ParaRes    =   1;
                    }
                    i++;
                    //---空床(在床)校准值--------4B
                    //pBuf[22]  -   pBuf[25]
                    i32=Count_4ByteToLong(pbuf1[i+3], pbuf1[i+2], pbuf1[i+1], pbuf1[i]);
                    if((i32!=0) && (i32!=0xFFFFFFFF) && (i32!=pPara->ThreshsholdValue2))
                    {
                        pPara->ThreshsholdValue2=i32;
                        Xkap_ParaRes    =   1;
                    ...
                    //---空床偏移量-在床---------1B
                    //pBuf[26]
                    if((pbuf1[i]!=0) && (pbuf1[i]!=0xFF) && (pbuf1[i]!=pPara->Offset_H[0]))
                    {
                        pPara->Offset_H[0]=pbuf1[i];
                        Xkap_ParaRes    =   1;
                    }
                    i++;
                    //---空床偏移量-离床---------1B
                    //pBuf[27]
                    if((pbuf1[i]!=0) && (pbuf1[i]!=0xFF) && (pbuf1[i]!=pPara->Offset_L[0]))
                    {
                        pPara->Offset_L[0]=pbuf1[i];
                        Xkap_ParaRes    =   1;
                    }
                    i++;
                    //---在床偏移量-在床---------1B
                    //pBuf[28]
                    if((pbuf1[i]!=0) && (pbuf1[i]!=0xFF) && (pbuf1[i]!=pPara->Offset_H[1]))
                    {
                        pPara->Offset_H[1]=pbuf1[i];
                        Xkap_ParaRes    =   1;
                    }
                    i++;
                    //---在床偏移量-离床---------1B
                    //pBuf[29]
                    if((pbuf1[i]!=0) && (pbuf1[i]!=0xFF) && (pbuf1[i]!=pPara->Offset_L[1]))
                    {
                        pPara->Offset_L[1]=pbuf1[i];
                        Xkap_ParaRes    =   1;
                    }
                    i++;
                    //---夜灯响应采集时长(100ms)-1B
                    //pBuf[30]
                    if((pbuf1[i]!=0) && (pbuf1[i]!=0xFF) && (pbuf1[i]!=pPara->Offset_TimerS))
                    {
                        pPara->Offset_TimerS=pbuf1[i];
                        Xkap_ParaRes    =   1;
                    }
                    i+=2;
                    //---夜灯响应时长(秒)--------2B
                    //pBuf[31]  -   pBuf[32]
                    crc2=Count_2ByteToWord(pbuf1[i+1], pbuf1[i]);
                    if(crc2!=pPara->Reponse_TimerS)
                    {
                        pPara->Reponse_TimerS=crc2;
                        Xkap_ParaRes    =   1;
