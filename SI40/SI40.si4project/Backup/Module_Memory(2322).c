...
static uint32_t memory_WeightBeginAddr=0,memory_WeightNum=0;
#endif
//------------------------------- 函数 ---------------------------------------
void Module_Memory_Init(void)
{
...
#if   (defined(XKAP_ICARE_B_M))
    {
        uint8_t i=0,j;
        uint32_t i32,j32;
        //计算memory_WeightNum初值
        for(i32=MODULE_MEMORY_EXTFLASH_WEIGHT_ADDR; i32<=MODULE_MEMORY_EXTFLASH_WEIGHT_ADDR_END; i32+=128)
        {
            Bsp_ExtFlash_Read_nByte(i32,(uint8_t*)&j,1);
            if(j!=0xFF)
            {
                memory_WeightNum++;
            }
        ...
        //计算memory_WeightBeginAddr初值
        if(memory_WeightNum==0||i==1)
        {
            memory_WeightBeginAddr=MODULE_MEMORY_EXTFLASH_WEIGHT_ADDR;
        }
        else
        {
            memory_WeightBeginAddr=j32;
        }
    }
#endif
}
uint8_t  Module_Memory_App(uint16_t Cmd,uint8_t *pBuf,uint8_t *pPara)
{
    uint8_t res=OK;
    uint32_t i32;
    MODULE_OS_ERR err;
    ...
    switch(Cmd)
    {
        ...
        case MODULE_MEMORY_APP_CMD_FACTORY:
        {
            ...
            if(*pPara==(uint8_t)MODULE_MEMORY_CMD_FACTORY_MODE_ALL)
            {
#if   (defined(MODULE_MEMORY_EXTFLASH_INIT))
                //清空外部FLASH
                ...
                psPara->Offset_H          =  0xFFFF;
                psPara->Offset_L          =  0xFFFF;
        ...
        case MODULE_MEMORY_APP_CMD_WEIGHT_R:
            // 读取索引
            i32=*((uint32_t*)pPara);
            if((i32>=(MODULE_MEMORY_EXTFLASH_WEIGHT_SIZE/128)))
            {
                res=ERR;
                break;
            }
            // 索引转换地址(0起始)
            i32 = memory_WeightBeginAddr+(i32*128);
            if(i32>=MODULE_MEMORY_EXTFLASH_WEIGHT_ADDR_END)
            {
                i32=i32-(MODULE_MEMORY_EXTFLASH_WEIGHT_ADDR_END+1);
            }
...
void Module_Memory_DebugTestOnOff(uint8_t OnOff)
{
    ...
    memory_WeightLock=1;
    {
        ...
        //
        sprintf((char*)pbuf,"BeginAddr: %ld(kB)\r\n",memory_WeightBeginAddr/1024);
        DebugOutStr((int8_t*)pbuf);
        sprintf((char*)pbuf,"Num      : %ld\r\n",memory_WeightNum);
        DebugOutStr((int8_t*)pbuf);
        //打印出存储的重量信息
        ps  =  (MODULE_MEMORY_S_WEIGHT *)pbuf;
        for(i32=0; i32<memory_WeightNum; i32++)
            //for(i32=0; i32<1440; i32++)
            //for(i32=0; i32<10; i32++)
        {
            ...
            for(i=0; i<10; i++)
            {
                if(i==0)
                {
                    sprintf((char*)&pbuf[128],"DateTime: %04d-%02d-%02d %02d:%02d:%02d %9ld %05d %05d %05d %05d\r\n"\
                            ,2000+ps->DataTime[0],ps->DataTime[1],ps->DataTime[2],ps->DataTime[3],ps->DataTime[4],ps->DataTime[5]\
                            ,ps->Hx711[i],ps->AdValue[0][i],ps->AdValue[1][i],ps->AdValue[2][i],ps->AdValue[3][i]);
                    DebugOutStr((int8_t*)&pbuf[128]);
                }
                else
                {
                    sprintf((char*)&pbuf[128],"                              %9ld %05d %05d %05d %05d\r\n"\
                            ,ps->Hx711[i],ps->AdValue[0][i],ps->AdValue[1][i],ps->AdValue[2][i],ps->AdValue[3][i]);
                    DebugOutStr((int8_t*)&pbuf[128]);
                }
...
void Module_Memory_DebugTestReadParaOnOff(uint8_t OnOff)
{
    ...
    DebugOutStr((int8_t*)pbuf);
    sprintf((char*)pbuf,"-> Offset_H          :0x%04X(%d)\r\n",ModuleMemory_psPara->Offset_H,ModuleMemory_psPara->Offset_H);
    DebugOutStr((int8_t*)pbuf);
    sprintf((char*)pbuf,"-> Offset_L          :0x%04X(%d)\r\n",ModuleMemory_psPara->Offset_L,ModuleMemory_psPara->Offset_L);
    DebugOutStr((int8_t*)pbuf);
