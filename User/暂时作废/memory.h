...
{
    INT8U Head[8];
    INT32U FalshAppAddr;  //应用地址
    INT8U SaveBlockNum;   //保存块编号(0起始,用于平衡擦除次数)
    //----------
    INT8U Reserve[1];     //保留
    INT16U Sum;
...
{
    INT16U ProductionDate;       //出厂年月
    INT16U SerialNumber;         //序号
    INT32U ProductCode;          //产品代码(名称)
    INT16U HardwareVer;          //硬件版本号
    INT16U SoftwareVer;          //软件版本号
    INT16U MemoryMainVer;        //存储结构主版本号
    INT16U MemorySubVer;         //存储结构子版本号
    INT8U Type;                  //门控器类型(1,2,4)
    INT8U addr;                  //地址(1-8)
    INT16U BaudRate;             //
    //----------
    INT8U Reserve[MEMORY_VER_SIZE-MEMORY_VER_LEN-2]; //保留
    INT16U Sum;                  //校验和
} S_MEMORY_VER;
//校验结构体(只用于校验)
...
{
    INT16U CardRecordNumber;           //刷卡记录数量
    INT16U AlarmRecordNumber;          //报警记录数量
    S_CHECK Door;
    S_CHECK DoorGroup;
    S_CHECK CardGroup;
    S_CHECK Alarm;
...
{
    INT16U TransferAlarmTxNum;         //发送计数
    INT16U TransferAlarmTxLastNum;     //上一次发送计数
    INT16U TransferEventTxNum;         //发送计数
    INT16U TransferEventTxLastNum;     //上一次发送计数
    INT8U MonitorSign[4];    
    INT32U DoorFirstTime[2];           //首卡刷卡Unix时间(0xFFFFFFFF无效)
    INT8U ReaderLockBitmap;            //读卡器锁定位图(0-未锁定,1-锁定)
...
{
    INT8U DeviceEnable;                 //设备使能
    INT8U ForceCountMode;               //胁迫模式
    INT8U ReaderEnableBitmap;           //韦根信号使能位图
    INT8U ReaderPasswordEnableBitmap;   //读卡器超级密码使能位图
    INT8U ReaderTimeGroupEnableBitmap;  //读卡器准进时段使能位图
    INT8U ReaderManyCardOpenDoorEnableBitmap; //读卡器多卡开门使能位图
    //----------
...
typedef struct S_MEMORY_DOOR
{
    //-----基本设置-----
    INT8U Number;                   //编号
    INT8U InDoorNumber[2];          //防反传门内区域编号
    INT8U OutDoorNumber[2];         //防反传门外区域编号
    INT8U FunctionSelect;           //功能选项
    INT8U DoorGroup[2];             //门所属门组(2字节表示一个门组)
    INT8U LongNoCloseDoorTimeS;     //长时间不关门报警时间
    //------------------
    INT8U Reserve[MEMORY_DOOR_SIZE-MEMORY_DOOR_LEN-2];              //备用
    INT16U Sum;                     //校验和
} S_MEMORY_DOOR;
//门组位图
/*
...
typedef struct S_MEMORY_CARDG_BITMAP
{
    ...
    INT8U Bitmap[128];   //位图
    //------------------
    INT8U Reserve[MEMORY_CARDGBITMAP_SIZE-MEMORY_CARDGBITMAP_LEN-2];              //备用
    INT16U Sum;                     //校验和
...
{
    INT8U Number;                    //编号
    INT8U InterfaceType;             //0-韦根26;1-韦根34
    INT8U AuthenticationType;        //0-卡,1-卡+密,2-卡/密,3-密,4-多卡,5-多卡+密
    INT8U PasswordRetryCount;        //密码重试次数
    INT8U PasswordMaxInputTime_s;    //密码最大输入时间(秒)
    INT8U Password[4];               //密码
    INT8U PasswordErrLockTime_s;     //密码错误锁定时间(秒)
    INT8U EffectTimeGroupNumber;     //有效时间域
    INT8U functionOption;            //功能选项
    INT8U FirstCardEffectTime_m[2];  //首卡有效时间(分钟)
    INT8U CardG1Num[2];              //卡组1编号
    INT8U CardG1Sum;                 //卡组1人数
    INT8U CardG2Num[2];              //卡组1编号
    INT8U CardG2Sum;                 //卡组1人数
    INT8U CardG3Num[2];              //卡组1编号
    INT8U CardG3Sum;                 //卡组1人数
    //------------------
    INT8U Reserve[MEMORY_READER_SIZE-MEMORY_READER_LEN-2];          //备用
    INT16U Sum;                      //校验和
} S_MEMORY_READER;
//继电器
typedef struct S_MEMORY_RELAY
{
    INT8U Number;                    //编号
    //-----继电器设置-----
    INT8U Relay_Mode;               //模式:0常开/1常闭
    INT8U Relay_OpenDoorKeepTime;   //开门保持时间
    INT8U Relay_ExtendOpenDoorTime; //延长开门时间
    //INT8U Relay_EffectTimeGroupNumber;           //继电器有效时间域组编号
    INT8U Relay_LinkageRelayGroupNumber_Global;  //关联继电器组_全局
    INT8U Relay_LinkageRelayGroupNumber_Local;   //关联继电器组-本地
    //------------------
    INT8U Reserve[MEMORY_RELAY_SIZE-MEMORY_RELAY_LEN-2];              //备用
    INT16U Sum;                     //校验和
} S_MEMORY_RELAY;
//输入
typedef struct S_MEMORY_INPUT
{
    INT8U Number;                    //编号
    //-----输入设置-----
    INT8U Input_LineModeType;                    //线模块类型
    INT8U Input_FunctionSelect;                  //b0有效状态+b1使能
    INT8U Input_DelayTime_s;                     //输入延迟时间
    INT8U Input_KeepTime_s;                      //输入保持时间
    INT8U Input_DisableTimeGroupNumber;          //输入禁用时间域
    INT8U Input_LinkageRelayGroupNumber_Global;  //关联继电器组_全局
    INT8U Input_LinkageRelayGroupNumber_Local;   //关联继电器组-本地
    //
    INT8U Reserve[MEMORY_INPUT_SIZE-MEMORY_INPUT_LEN-2];              //备用
    INT16U Sum;                     //校验和
} S_MEMORY_INPUT;
//报警
typedef struct S_MEMORY_ALARM
{
    INT8U Common_RelayNumber;                     //常规报警-继电器序号
    INT8U Common_LinkageRelayGroupNumber_Global;  //常规报警-联动全局继电器组编号
    INT8U Common_LinkageRelayGroupNumber_Local;   //常规报警-联动本地继电器组编号
    INT8U Force_RelayNumber;                      //胁迫报警-继电器序号
    INT8U Force_LinkageRelayGroupNumber_Global;   //胁迫报警-联动全局继电器组编号
    INT8U Force_LinkageRelayGroupNumber_Local;    //胁迫报警-联动本地继电器组编号
    INT8U Invade_RelayNumber;                     //入侵报警-继电器序号
    INT8U Invade_LinkageRelayGroupNumber_Global;  //入侵报警-联动全局继电器组编号
    INT8U Invade_LinkageRelayGroupNumber_Local;   //入侵报警-联动本地继电器组编号
    INT8U Fault_RelayNumber;                      //故障报警-继电器序号
    INT8U Fault_LinkageRelayGroupNumber_Global;   //故障报警-联动全局继电器组编号
    INT8U Fault_LinkageRelayGroupNumber_Local;    //故障报警-联动本地继电器组编号
    INT8U Reverse_FunctionSelect;                 //b0反传-事件报告
    //b1反传-报警
    //b2反传报警-联动本地继电器组编号
    //----------
...
{
    INT8U Number[2];                //编号
    INT8U FunctionSelect;        //b0:门组内互锁,b1-b7:保留
    //----------
...
{
    INT8U Number[2];                   //编号
    INT8U AreaGroupNumber[4];       //区域组编号
    INT8U WayId[2];                 //线路ID
    INT8U RedioSAB;                 //防潜回功能单选
    INT8U TimeGroupNumber;          //时间域组编号
    //----------
    INT8U Reserve[MEMORY_CARDGROUPLIMIT_SIZE-MEMORY_CARDGROUPLIMIT_LEN-2];          //备用
    INT16U Sum;                     //校验和
...
{
    INT8U Number[2];                 //编号
    INT8U AreaGroupNumber;        //区域组编号
    INT8U TimeGroupNumber;        //时间域组ID
    INT8U SafetyLevel;            //安全等级
    INT8U PeopleMax[2];             //人数上限
    INT8U PeopleMin[2];             //人数下限
    INT8U FunctionSelect;         //功能选项:
    //b0-区域内门间互锁使能
    //b1-区域组内互锁使能
    //----------
    INT8U InterlockArea[2];       //互锁区域
...
{
    INT8U Number;                //编号    
    INT8U HolidayGroupNumber;    //节假日组编号
    INT8U EffectBeginTime[5];    //时标
    INT8U EffectEndTime[5];      //时标
    INT8U TimeBitmap[7][12];     //周n工作时间位图(96bit,15min/bit)
    //----------
...
{
    INT8U CardNumber[4];        //卡号
    INT8U CardGroupNumber[2];    //卡组编号
    INT8U EffectBeginTime[5]; //时标
    INT8U EffectEndTime[5];   //时标
    INT8U Password[4];        //
    INT8U Status;             //状态(无/0-有效/1-丢失/2-损坏/3-禁用/4-待用/5-黑名单 0xFF-已被删除)
    INT8U Type;               //卡类型
    INT8U Privilege[2];       //特权
    INT8U PassLevel;          //通行等级
    INT8U ForceLevel;         //胁迫等级
    //----------
    INT8U Reserve[MEMORY_FLASH_USER_PERMISSION_SIZE-MEMORY_FLASH_USER_PERMISSION_LEN-2];   //保留
    INT16U Sum;
...
{
    //INT8U Number[2];             //编号
    INT8U AlarmArea[2];          //报警区域
    INT8U ReaderOrWireOrRelayNum;//读卡器/线模块/继电器编号
    INT8U CardNumber[4];         //卡号
    INT8U TargetArea[2];         //目标区域
    INT8U CurrentArea[2];        //当前区域(门控器固定赋值为0)
    INT8U Event[2];              //事件(包含卡片状态)
    INT8U TimeGErr;              //违反的时间域组
    INT8U RecordTime[5];         //时间
    //----------
...
{
   INT8U Number;                 //编号  
   INT8U Input1Type;             //输入1的触发状态(0-始终有效,1-低电平,2-高电平,3-下降沿,4-上升沿)
   INT8U Input1Time_100ms;       //输入1的保持/去抖时间(100mS)
   INT8U Input2Type;             //输入2的触发状态(0-始终有效,1-低电平,2-高电平,3-下降沿,4-上升沿)
   INT8U Input2Time_100ms;       //输入2的保持/去抖时间(100mS)
   INT8U Input3Type;             //输入3的触发状态(0-始终有效,1-低电平,2-高电平,3-下降沿,4-上升沿)
   INT8U Input3Time_100ms;       //输入3的保持/去抖时间(100mS)
   INT8U Input4Type;             //输入4的触发状态(0-始终有效,1-低电平,2-高电平,3-下降沿,4-上升沿)
   INT8U Input4Time_100ms;       //输入4的保持/去抖时间(100mS)
   INT8U Output1OnTimeS[2];      //输出1的开启时长(秒)
   INT8U Output1_TimeG;          //输出1的有效时间域组
   INT8U Output2OnTimeS[2];      //输出2的开启时长(秒)
   INT8U Output2_TimeG;          //输出2的有效时间域组
   INT8U Output3OnTimeS[2];      //输出3的开启时长(秒)
   INT8U Output3_TimeG;          //输出3的有效时间域组
   INT8U Output4OnTimeS[2];      //输出4的开启时长(秒)
   INT8U Output4_TimeG;          //输出4的有效时间域组
...
{
    INT8U CardNumber[4];    //卡号
    INT8U State;            //0-未知,1-存在,2-不存在,3-已满
} S_MEMORY_REVERSE;
//
