...
void MPU6050_WriteByte(uint8_t _ucRegAddr, uint8_t _ucRegData)
{
    ...
    BspGpioIIC_SendByte(MPU6050_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	BspGpioIIC_WaitAck();

    BspGpioIIC_SendByte(_ucRegAddr);				/* 内部寄存器地址 */
	BspGpioIIC_WaitAck();

    BspGpioIIC_SendByte(_ucRegData);				/* 内部寄存器数据 */
	BspGpioIIC_WaitAck();
...
uint8_t MPU6050_ReadByte(uint8_t _ucRegAddr)
{
	...
	BspGpioIIC_SendByte(MPU6050_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	BspGpioIIC_WaitAck();
	BspGpioIIC_SendByte(_ucRegAddr);     			/* 发送存储单元地址 */
	BspGpioIIC_WaitAck();

	BspGpioIIC_Start();                  			/* 总线开始信号 */

	BspGpioIIC_SendByte(MPU6050_SLAVE_ADDRESS+1); 	/* 发送设备地址+读信号 */
	BspGpioIIC_WaitAck();

	ucData = BspGpioIIC_ReadByte();       			/* 读出寄存器数据 */
	BspGpioIIC_NAck();
	BspGpioIIC_Stop();                  			/* 总线停止信号 */
	return ucData;
}
...
void MPU6050_ReadData(void)
{
	...
	BspGpioIIC_SendByte(MPU6050_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	BspGpioIIC_WaitAck();
	BspGpioIIC_SendByte(ACCEL_XOUT_H);     		/* 发送存储单元地址  */
	BspGpioIIC_WaitAck();
	...
	for (i = 0; i < 13; i++)
	{
		ucReadBuf[i] = BspGpioIIC_ReadByte();       			/* 读出寄存器数据 */
		BspGpioIIC_Ack();
	}

	/* 读最后一个字节，时给 NAck */
	ucReadBuf[13] = BspGpioIIC_ReadByte();
	BspGpioIIC_NAck();
