...
void HMC5883L_WriteByte(uint8_t _ucRegAddr, uint8_t _ucRegData)
{
    ...
    BspGpioIIC_SendByte(HMC5883L_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	BspGpioIIC_WaitAck();
...
uint8_t HMC5883L_ReadByte(uint8_t _ucRegAddr)
{
	...
	BspGpioIIC_SendByte(HMC5883L_SLAVE_ADDRESS+1); 	/* 发送设备地址+读信号 */
	BspGpioIIC_WaitAck();
...
void HMC5883L_ReadData(void)
{
	...
	BspGpioIIC_SendByte(HMC5883L_SLAVE_ADDRESS);	/* 发送设备地址+写信号 */
	BspGpioIIC_WaitAck();
	BspGpioIIC_SendByte(DATA_OUT_X);     		/* 发送存储单元地址  */
	BspGpioIIC_WaitAck();
	...
	/* 读最后一个字节，时给 NAck */
	ucReadBuf[6] = BspGpioIIC_ReadByte();
	BspGpioIIC_NAck();
