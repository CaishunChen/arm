...
static void BMP085_WriteReg(uint8_t _ucRegAddr, uint8_t _ucRegValue)
{
    ...
    BspGpioIIC_SendByte(BMP085_SLAVE_ADDRESS);		/* 发送设备地址+写信号 */
	BspGpioIIC_WaitAck();
...
static uint16_t BMP085_Read2Bytes(uint8_t _ucRegAddr)
{
	...
	BspGpioIIC_Start();                  			/* 总线开始信号 */
	BspGpioIIC_SendByte(BMP085_SLAVE_ADDRESS + 1);/* 发送设备地址+读信号 */
	BspGpioIIC_WaitAck();
...
static uint32_t BMP085_Read3Bytes(uint8_t _ucRegAddr)
{
	...
	ucData2 = BspGpioIIC_ReadByte();       		/* 读出中间字节数据 */
	BspGpioIIC_Ack();

	ucData3 = BspGpioIIC_ReadByte();       		/* 读出最低节数据 */
	BspGpioIIC_NAck();
	BspGpioIIC_Stop();                  			/* 总线停止信号 */
