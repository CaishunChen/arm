...
uint8_t SI4730_SendCmd(uint8_t *_pCmdBuf, uint8_t _ucCmdLen)
{
	...
	BspGpioIIC_Start();
	BspGpioIIC_SendByte(I2C_ADDR_SI4730_W);
	ack = BspGpioIIC_WaitAck();
	if (ack != 0)
	{
		goto err;
	}
	...
	{
		BspGpioIIC_SendByte(_pCmdBuf[i]);
		ack = BspGpioIIC_WaitAck();
		if (ack != 0)
		{
			goto err;
...
uint32_t SI4730_WaitStatus80(uint32_t _uiTimeOut, uint8_t _ucStopEn)
{
	...
	{
		BspGpioIIC_Start();
		BspGpioIIC_SendByte(I2C_ADDR_SI4730_R);	/* 读 */
		ack = BspGpioIIC_WaitAck();
		if (ack == 1)
		{
			BspGpioIIC_NAck();
...
uint8_t SI4730_GetRevision(uint8_t *_ReadBuf)
{
	...
	*/
	...
	{
		BspGpioIIC_Ack();
		_ReadBuf[i] = BspGpioIIC_ReadByte();
	}
	BspGpioIIC_NAck();
	BspGpioIIC_Stop();
	return 1;
