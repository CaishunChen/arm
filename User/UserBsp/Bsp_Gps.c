...
uint8_t Gps_DebugTest_Enable=0;
GPS_T g_tGPS;
...
#endif
    GPIO_InitStructure.GPIO_Pin     =  BSP_GPS_PWR_PIN;
    GPIO_Init(BSP_GPS_PWR_PORT, &GPIO_InitStructure);
    BSP_GPS_PWR_ON;
#endif
    //硬件初始化
    UART_INIT(BSP_GPS_UARTX,9600);
    //数据初始化
    g_tGPS.PositionOk = 0;
    g_tGPS.TimeOk = 0;
    g_tGPS.UartOk = 0;
...
void Gps_DebugTestOnOff(uint8_t OnOff)
{
    if(OnOff==ON)
    {
        Gps_DebugTest_Enable=1;
    }
    else
    {
        Gps_DebugTest_Enable=0;
    }
}
