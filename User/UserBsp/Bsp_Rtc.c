...
//延时复位(0无效)
uint8_t    BspRtc_SoftResetTime_s	=	0;
//
#if (RTC_1000MS_IN_INTERRUPT==0)
static uint8_t  BspRtc_Flag_1S  =   0;
#endif
...
void BspRtc_Init(void)
{
    ...
    // 获取闹钟时间
    {
        ...
        {
            Rtc_Alarm[err]       =  ModuleMemory_psPara->RtcAlarm[err];
            Rtc_AlarmMode[err]   =  ModuleMemory_psPara->RtcAlarmMode[err];
        }
        //提醒
        for(err=0; err<BSP_RTC_WARN_MAX_NUM; err++)
        {
            Rtc_Warn_OnOff[err]  =  ModuleMemory_psPara->RtcWarnOnOff[err];
            Rtc_Warn_Time[err]   =  ModuleMemory_psPara->RtcWarnTime[err];
        }
        //自启动
    }
    //月
...
void BspRtc_SetAuto(uint8_t OnOff,uint16_t BeginTime,uint16_t EndTime)
{
    ...
    Module_Memory_App(MODULE_MEMORY_APP_CMD_GLOBAL_W,(uint8_t*)pPara,NULL);
    MemManager_Free(E_MEM_MANAGER_TYPE_256B,pPara);
    if(ModuleMemory_psPara->RtcAutoOnOff==OFF)
    {
        Rtc_Auto_State=ON;
    }
#else
...
void BspRtc_1000ms(void)
{
    ...
    Rtc_BeginToNow_s++;
	// 延时软复位
    if(BspRtc_SoftResetTime_s!=0)
    {
        BspRtc_SoftResetTime_s--;
        if(BspRtc_SoftResetTime_s==0)
        {
            MCU_SOFT_RESET;
        }
    ...
    if(ModuleMemory_psPara->RtcAutoOnOff ==ON)
    {
        ...
        //起始时间
        j32 = ((ModuleMemory_psPara->RtcAutoBeginTime)>>8)*3600+((ModuleMemory_psPara->RtcAutoBeginTime)&0x00FF)*60;
        j32 = i32-j32;
        ...
        //结束时间
        j32 = ((ModuleMemory_psPara->RtcAutoEndTime)>>8)*3600+((ModuleMemory_psPara->RtcAutoEndTime)&0x00FF)*60;
        j32 = i32-j32;
...
uint8_t BspRtc_ReadRealTime(uint8_t *ptime5,struct tm *pCurrentTime_tm,time_t *pUnixTime,uint8_t *ptime6)
{
    ...
    {
        //年
        ptime5[4]=(BspRtc_CurrentTimeBuf[0]<<1);
        //月
        ptime5[4]+=(BspRtc_CurrentTimeBuf[1]>>3);
        ptime5[3]=(BspRtc_CurrentTimeBuf[1]<<5);
        //日
        ptime5[3]+=BspRtc_CurrentTimeBuf[2];
        //时
        ptime5[2]=(BspRtc_CurrentTimeBuf[3]<<3);
        //分
        ptime5[2]+=(BspRtc_CurrentTimeBuf[4]>>3);
        ptime5[1]=(BspRtc_CurrentTimeBuf[4]<<5);
        //秒
        ptime5[1]+=(BspRtc_CurrentTimeBuf[5]>>1);
        ptime5[0]=(BspRtc_CurrentTimeBuf[5]<<7);
        //毫秒
    }
    ...
    if(ptime6!=NULL)
    {
        ptime6[0]=BspRtc_CurrentTimeBuf[0];
        ptime6[1]=BspRtc_CurrentTimeBuf[1];
        ptime6[2]=BspRtc_CurrentTimeBuf[2];
        ptime6[3]=BspRtc_CurrentTimeBuf[3];
        ptime6[4]=BspRtc_CurrentTimeBuf[4];
        ptime6[5]=BspRtc_CurrentTimeBuf[5];
    }
    return OK;
}
