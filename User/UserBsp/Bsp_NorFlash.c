...
void BspNorFlash_DebugTestOnOff(uint8_t OnOff)
{
   ...
   // 测试芯片信息
   sprintf((char*)pbuf,"*Nor Flash Chip Test:\r\n");
   DebugOutStr((int8_t*)pbuf);
   i32 = NOR_ReadID();
   if (i32 == S29GL128P)
   {
      sprintf((char*)pbuf,"  Chip ID = 0x%08X, Model = S29GL128P\r\n", i32);
      DebugOutStr((int8_t*)pbuf);
   }
   else
   {
      sprintf((char*)pbuf,"  Chip ID = 0x%08X, Model = xxxx \r\n", i32);
      DebugOutStr((int8_t*)pbuf);
   }
   // 擦出整个芯片
   /*
   sprintf((char*)pbuf,"*Erase Whole Nor Flash Chip:\r\n");
   DebugOutStr((int8_t*)pbuf);
	sprintf((char*)pbuf,"  Begin,please wait...(about 45 second)\r\n");
   DebugOutStr((int8_t*)pbuf);
   Count_GetTimeMs(&i32);
	pbuf[0] = NOR_EraseChip();
	Count_GetTimeMs(&j32);
	if (pbuf[0] == NOR_SUCCESS)
	{
		sprintf((char*)pbuf,"  Erase OK: %dms\r\n", j32 - i32);
	}
	else
	{
		sprintf((char*)pbuf,"  Erase ERR: %dms\r\n", j32 - i32);
	}
   DebugOutStr((int8_t*)pbuf);
   */
   // 擦出指定扇区(0-127)
   sprintf((char*)pbuf,"*Erase sector(No.%d) (%dK Byte)\r\n",  127 + 1, NOR_SECTOR_SIZE / 1024);
   DebugOutStr((int8_t*)pbuf);
	Count_GetTimeMs(&i32);
	pbuf[0] = NOR_EraseSector(127 * NOR_SECTOR_SIZE);
	Count_GetTimeMs(&j32);
	if (pbuf[0] == NOR_SUCCESS)
	{
		sprintf((char*)pbuf,"  Erase sector(No.%d) OK: %dms\r\n", 127 + 1, j32 - i32);
	}
	else
	{
		sprintf((char*)pbuf,"  Erase sector(No.%d) ERR: %dms\r\n", 127 + 1, j32 - i32);
	}
   DebugOutStr((int8_t*)pbuf);
   // 编程-单字节
   sprintf((char*)pbuf,"*Write 1 Byte to secort(No.%d),offset 0!\r\n", 127 + 1);
   DebugOutStr((int8_t*)pbuf);
	{
      if (NOR_WriteByte(127 * NOR_SECTOR_SIZE, 0xA5) == NOR_SUCCESS)
      {
         sprintf((char*)pbuf,"  Write OK!\r\n");
      }
      else
      {
         sprintf((char*)pbuf,"  Write ERR!\r\n");
      }
   }
   DebugOutStr((int8_t*)pbuf);
   // 编程-多字节
   sprintf((char*)pbuf,"*Write n Byte to secort(No.%d), offset 1\r\n", 127 + 1);
   DebugOutStr((int8_t*)pbuf);
	{
		for(pbuf[250]=0;pbuf[250]<100;pbuf[250]++)
		{
         pbuf[100+pbuf[250]]=pbuf[250];
      }
		if (NOR_WriteBuffer(&pbuf[100], 127 * NOR_SECTOR_SIZE + 1, 100) == NOR_SUCCESS)
		{
			sprintf((char*)pbuf,"  Write OK!\r\n");
		}
		else
   ...
   DebugOutStr((int8_t*)pbuf);
   // 编程-扇区
   sprintf((char*)pbuf,"*Write whole secort(No.%d)(test speed)\r\n", 127 + 1);
   DebugOutStr((int8_t*)pbuf);
   pbuf1 = MemManager_Get(E_MEM_MANAGER_TYPE_1KB);
	memset(pbuf1, 0xA5, MEM_1KB_BLK_SIZE);
	j32 = 127 * NOR_SECTOR_SIZE;
	Count_GetTimeMs(&i32);
	for (pbuf[201] = 0; pbuf[201] < NOR_SECTOR_SIZE / MEM_1KB_BLK_SIZE; pbuf[201]++)
	{
		pbuf[200] = NOR_WriteBuffer(pbuf1, j32, MEM_1KB_BLK_SIZE);
		if (pbuf[200] != NOR_SUCCESS)
		{
			break;
		}
		j32 += MEM_1KB_BLK_SIZE;
	}
	Count_GetTimeMs(&j32);

	if (pbuf[200] == NOR_SUCCESS)
	{
		sprintf((char*)pbuf,"  Write OK : %dms, %dKB/S\r\n", j32-i32,((NOR_SECTOR_SIZE / 1024) * 1000) / (j32-i32));
	}
	else
	{
		sprintf((char*)pbuf,"  Write ERR: %dms\r\n", j32-i32);
	}
   DebugOutStr((int8_t*)pbuf);
   MemManager_Free(E_MEM_MANAGER_TYPE_1KB,pbuf1);
   // 读取-数据
   sprintf((char*)pbuf,"*Read 1KB from No.%d sector\r\n", 127 + 1);
   DebugOutStr((int8_t*)pbuf);
	pbuf2 = (uint16_t *)(NOR_FLASH_ADDR + (127*NOR_SECTOR_SIZE));
	sprintf((char*)pbuf,"  No.%d sector,offset addr:0x%08X,Size: %dByte,data: \r\n", (127*NOR_SECTOR_SIZE) / NOR_SECTOR_SIZE + 1
		, (127*NOR_SECTOR_SIZE), 1024);
	for (i32 = 0; i32 < 1024 / 2; i32++)
	{
		j32 = *pbuf2++;
		printf(" %02X %02X", (j32 & 0xFF), j32 >> 8);
   ...
   // 
   sprintf((char*)pbuf,"*Read whole chip (test speed)\r\n");
   DebugOutStr((int8_t*)pbuf);
	pbuf2 = (uint16_t *)(NOR_FLASH_ADDR);
	Count_GetTimeMs(&i32);
	for (j32 = 0; j32 < NOR_FLASH_SIZE / 2; j32++)
	{
		i16 = *pbuf2++;
	}
   i16=i16;
	...
	sprintf((char*)pbuf,"  Read whole chip OK ,16MByte: %dms, Speed: %dKB/S\r\n", j32 - i32,
			((NOR_FLASH_SIZE / 1024) * 1000) / (j32 - i32));
   DebugOutStr((int8_t*)pbuf);
   //
