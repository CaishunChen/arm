...
uint8 DHCPBUF[1024];
DHCPXSL_S_MSG*  pRIPMSG = (DHCPXSL_S_MSG*)DHCPBUF;         // Pointer for the DHCP message
...
void send_DHCP_DISCOVER(void)
{
  ...
  memset((void*)pRIPMSG,0,sizeof(DHCPXSL_S_MSG));
  ...
  //printf("send dhcp discover %d\r\n",sizeof(DHCPXSL_S_MSG));

  sendto(SOCK_DHCP, (uint8 *)pRIPMSG, sizeof(DHCPXSL_S_MSG), ip, DHCP_SERVER_PORT);	
...
void send_DHCP_REQUEST(void)
{
  ...
  //printf("send dhcp request\r\n");
    sendto(SOCK_DHCP, (const uint8 *)pRIPMSG, sizeof(DHCPXSL_S_MSG), ip, DHCP_SERVER_PORT);
  //if(len==0)
