...
/* Private typedef -----------------------------------------------------------*/
typedef struct ModuleDList_S_Node
{
    //键值(用于排序与查找)
    uint32_t KeyValue;
    //链表下一个结点
    struct ModuleDList_S_Node *pNextNode;
    //链表前一个结点
    struct ModuleDList_S_Node *pPrevNode;
    //-----
    //自身绝对地址
    struct ModuleDList_S_Node *pSelfNode;
    //参数指针
    void *pPara;
}ModuleDList_S_Node;
typedef struct ModuleDList_S_Para
{
	uint8_t len;
	uint8_t pbuf[16-1];
}ModuleDList_S_Para;
/* Private define ------------------------------------------------------------*/
#define MODULE_DLIST_NODE_NUM_MAX           20480   //  每个结点20Byte 20480占用400kB空间


#define MODULE_DLIST_LIST_MEM_ADDR_BEGIN    0
#define MODULE_DLIST_LIST_MEM_SIZE          sizeof(ModuleDList_S_Node)    //  链表每单元去占用空间
#define MODULE_DLIST_LIST_MEM_ADDR_END      MODULE_DLIST_LIST_MEM_ADDR_BEGIN+(MODULE_DLIST_NODE_NUM_MAX*sizeof(ModuleDList_S_Node))-1
...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static uint32_t ModuleDList_ListMem_Num     =   0;  //  有效数量        0起始
...
void ModuleDList_Mem_Init(void)
{
    ...
    {
        // 提取结构体
        ps_extFlash->addr   =   MODULE_DLIST_LIST_MEM_ADDR_BEGIN    +   i32*sizeof(ModuleDList_S_Node);
        ps_extFlash->len    =   sizeof(ModuleDList_S_Node);
        Module_Memory_App(MODULE_MEMORY_APP_CMD_EXTFLASH_READ,(uint8_t*)ps_node,(uint8_t*)ps_extFlash);
        if(ps_node->pPara!=(void *)0xFFFFFFFF)
        {
 ...
 * @brief   获取链表空间
 * @note
 * @param   pNode - 已分配空间的结点指针
 * @return  OK ERR
 */
uint8_t ModuleDList_Get(ModuleDList_S_Node* pNode)
{
    // 如果 ModuleDList_ListMem_Num 未达到最大，则向后取空间
    if(ModuleDList_ListMem_EndNum < MODULE_DLIST_NODE_NUM_MAX)
    {
        pNode->pSelfNode = (ModuleDList_S_Node*)(MODULE_DLIST_LIST_MEM_ADDR_BEGIN +   ModuleDList_ListMem_EndNum*sizeof(ModuleDList_S_Node));
...
uint8_t ModuleDList_Save(ModuleDList_S_Node* pNode)
{
    MODULE_MEMORY_S_EXTFLASH *ps_extFlash;
    if(
#if		(MODULE_DLIST_LIST_MEM_ADDR_BEGIN!=0)		
		((uint32_t)(pNode->pSelfNode)<MODULE_DLIST_LIST_MEM_ADDR_BEGIN)||
#endif		
		((uint32_t)(pNode->pSelfNode)>MODULE_DLIST_LIST_MEM_ADDR_END))
    {
        return ERR;
    }
    ps_extFlash     =   MemManager_Get(E_MEM_MANAGER_TYPE_256B);
    ps_extFlash->addr   =   (uint32_t)(pNode->pSelfNode);
    ps_extFlash->len    =   sizeof(ModuleDList_S_Node);
    Module_Memory_App(MODULE_MEMORY_APP_CMD_EXTFLASH_WRITE,(uint8_t*)pNode,(uint8_t*)ps_extFlash);
    MemManager_Free(E_MEM_MANAGER_TYPE_256B, ps_extFlash);
    return OK;
}
/**
 * @brief   释放链表空间
 ...
 * @return  None
 */
uint8_t ModuleDList_Free(ModuleDList_S_Node* pNode)
{
    ModuleDList_S_Node *ps_node;
	ModuleDList_S_Para *ps_para;
    MODULE_MEMORY_S_EXTFLASH *ps_extFlash;
    ps_node     =   MemManager_Get(E_MEM_MANAGER_TYPE_256B);
    ps_extFlash =   (MODULE_MEMORY_S_EXTFLASH*)&ps_node[1];
	ps_para		=	(ModuleDList_S_Para*)&ps_node[2];
    // 释放参数区(全部清0)
    ps_extFlash->addr   =   (uint32_t)(pNode->pPara);
    ps_extFlash->len    =   MODULE_DLIST_PARA_MEM_SIZE;
	memset((char*)ps_para,0,sizeof(ModuleDList_S_Para));
	Module_Memory_App(MODULE_MEMORY_APP_CMD_EXTFLASH_WRITE,(uint8_t*)ps_para,(uint8_t*)ps_extFlash);
    // 释放链表区(全部清0)
    ps_extFlash->addr   =   (uint32_t)(pNode->pSelfNode);
    ps_extFlash->len    =   MODULE_DLIST_LIST_MEM_SIZE;
    memset((char*)ps_node,0,sizeof(ModuleDList_S_Node));
...
*********************************************************************************
*/
void ModuleDList_InitList(ModuleDList_S_Node **pNode)
{
    *pNode=NULL;
...
*********************************************************************************
*/
ModuleDList_S_Node *ModuleDList_CreatList(ModuleDList_S_Node *pHead,uint32_t KeyValue)
{
    ModuleDList_S_Node *p1,*p2;
    p1=p2=(ModuleDList_S_Node *)malloc(sizeof(ModuleDList_S_Node));
    if(p1 == NULL || p2 ==NULL)
    {
        if(ModuleDList_DebugTest_Enable==1)
        {
    ...
    memset(p1,0,sizeof(ModuleDList_S_Node));
    ...
    while(p1->KeyValue >0)
    {
        ...
        p2=p1;
        p1=(ModuleDList_S_Node *)malloc(sizeof(ModuleDList_S_Node));
...
*********************************************************************************
*/
void ModuleDList_PrintList(ModuleDList_S_Node *pHead)
{
    if(NULL==pHead)
    {
        if(ModuleDList_DebugTest_Enable==1)
...
*********************************************************************************
*/
void ModuleDList_ClearList(ModuleDList_S_Node *pHead)
{
    ModuleDList_S_Node *pNext;
    if(pHead==NULL)
    {
        if(ModuleDList_DebugTest_Enable==1)
        {
...
*********************************************************************************
*/
int32_t ModuleDList_SizeList(ModuleDList_S_Node *pHead)
{
    int32_t size=0;
    while(pHead!=NULL)
    {
...
*********************************************************************************
*/
int32_t ModuleDList_IsEmptyList(ModuleDList_S_Node *pHead)
{
    if(pHead==NULL)
    {
        if(ModuleDList_DebugTest_Enable==1)
...
*********************************************************************************
*/
int32_t ModuleDList_GetElement(ModuleDList_S_Node *pHead,int32_t pos)
{
    int32_t i=0;
    if(pos<1)
    {
...
*********************************************************************************
*/
uint32_t *ModuleDList_GetElemAddr(ModuleDList_S_Node *pHead,uint32_t KeyValue)
{
    if(NULL==pHead)
    {
        if(ModuleDList_DebugTest_Enable==1)
...
*********************************************************************************
*/
int32_t ModuleDList_ModifyElem(ModuleDList_S_Node *pNode,int32_t pos,uint32_t KeyValue)
{
    int32_t i=0;
    ModuleDList_S_Node *pHead;
    pHead=pNode;
    if(NULL==pHead)
    {
        if(ModuleDList_DebugTest_Enable==1)
...
*********************************************************************************
*/
int32_t ModuleDList_InsertHeadList(ModuleDList_S_Node **pNode,uint32_t KeyValue)
{
    ModuleDList_S_Node *pInsert;
    pInsert=(ModuleDList_S_Node *)malloc(sizeof(ModuleDList_S_Node));
    if(pInsert==NULL)
    {
        return NULL;
    }
    memset(pInsert,0,sizeof(ModuleDList_S_Node));
...
*********************************************************************************
*/
int32_t ModuleDList_InsertLastList(ModuleDList_S_Node *pNode,uint32_t KeyValue)
{
    ModuleDList_S_Node *pInsert;
    ModuleDList_S_Node *pHead;
    //Node *pTmp;
...
*********************************************************************************
*/
int32_t ModuleDList_IsAddPos(ModuleDList_S_Node *pNode,int32_t pos,uint32_t KeyValue)
{
    ModuleDList_S_Node *pHead=pNode;
    ModuleDList_S_Node *pTmp;
    ...
    // 申请缓存
    pTmp=(ModuleDList_S_Node *)malloc(sizeof(ModuleDList_S_Node));
    if(pTmp==NULL)
    {
        return NULL;
    }
    memset(pTmp,0,sizeof(ModuleDList_S_Node));
    // 更新next指针
...
*********************************************************************************
*/
int32_t ModuleDList_OrrderList(ModuleDList_S_Node *pNode,uint32_t KeyValue)
{
...
*********************************************************************************
*/
int32_t ModuleDList_DelHeadList(ModuleDList_S_Node **pList)
{
    ModuleDList_S_Node *pHead;
    pHead=*pList;
    if(pHead!=NULL)
...
*********************************************************************************
*/
int32_t ModuleDList_DelLastList(ModuleDList_S_Node *pNode)
{
    ModuleDList_S_Node *pHead=pNode;
    ModuleDList_S_Node *pTmp;
...
*********************************************************************************
*/
int32_t ModuleDList_DelPos(ModuleDList_S_Node *pNode,int32_t pos)
{
    ModuleDList_S_Node *pHead=pNode;
    //Node *pTmp;
...
*********************************************************************************
*/
int32_t ModuleDList_Delx(ModuleDList_S_Node **pNode,int32_t KeyValue)
{
    ModuleDList_S_Node *pHead=*pNode;
    //Node *pTmp;
    int32_t i=0;
...
*********************************************************************************
*/
int32_t ModuleDList_Exchange2Pos(ModuleDList_S_Node *pNode,int32_t pos1,int32_t pos2)
{
    ModuleDList_S_Node *pNodePos1,*pNodePos2;
    //uint32_t *pTmp;
    //uint32_t *pInsert;
    //uint32_t a;
    int32_t i=0;
    ModuleDList_S_Node temp_node;
...
*********************************************************************************
*/
static int32_t ModuleDList_Swap(ModuleDList_S_Node *pNodePos1,ModuleDList_S_Node *pNodePos2)
{
    ModuleDList_S_Node temp_node;
...
int32_t ModuleDList_Arrange(ModuleDList_S_Node *pNode)
{
    ModuleDList_S_Node *pHead=pNode;
    int32_t a=0,i,j;
