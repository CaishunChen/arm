...
//
static uint8_t ModuleRBTree_DebugTest_Enable=0;
//
static rb_node_t* rb_new_node(key_t key,data_t data)
{
    ...
    if (!node)
    {
        if(ModuleRBTree_DebugTest_Enable==1)
        {
            ModuleRBTree_DebugPrintf("malloc error!\r\n");
        }
        return NULL;
    }
...
rb_node_t* rb_erase(key_t key, rb_node_t *root)
{
    ...
    if (!(node == rb_search_auxiliary(key, root, NULL)))  //调用rb_search_auxiliary查找要删除的结点
    {
        if(ModuleRBTree_DebugTest_Enable==1)
        {
            ModuleRBTree_DebugPrintf("key is not exist!\r\n");
        }
        return root;
    }
...
void ModuleRBTree_DebugTestOnOff(uint8_t OnOff)
{
    ...
    ModuleRBTree_DebugTest_Enable=1;
    for (i = 1; i < count; ++i)
    {
        key = Count_Rand() % count;
        //插入
        /*
        if ((root == rb_insert(key, i, root)))
        {
            sprintf((char*)pbuf,"[i = %d] insert key %d success!\n", i, key);
            ModuleRBTree_DebugPrintf(pbuf);
        }
        else
        {
            sprintf((char*)pbuf,"[i = %d] insert key %d error!\n", i, key);
            ModuleRBTree_DebugPrintf(pbuf);
            break;
        }
        */
        ...
        //搜索
        node = NULL;
        node == rb_search(key, root);
        if (node != NULL)
        {
            sprintf((char*)pbuf,"[i = %d] search key %d success!\n", i, key);
            ModuleRBTree_DebugPrintf(pbuf);
        }
        else
        {
            sprintf((char*)pbuf,"[i = %d] search key %d error!\n", i, key);
            ModuleRBTree_DebugPrintf(pbuf);
            break;
        }
        //
        if (!(i % 10))
        {
            ...
            {
                sprintf((char*)pbuf,"[i = %d] erase key %d success\n", i, key);
                ModuleRBTree_DebugPrintf(pbuf);
            }
            else
            {
                sprintf((char*)pbuf,"[i = %d] erase key %d error\n", i, key);
                ModuleRBTree_DebugPrintf(pbuf);
            }
            */
    ...
    ModuleRBTree_DebugTest_Enable=0;
