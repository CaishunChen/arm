...
/*************************************说明*****************************************
// 文件描述： 应用层存储读写函数
// 注意事项： 目前只有FLASH操作的卡权限修改部分有断电恢复功能.其他部分待完善.
**********************************************************************************/
//-------------------------------------------------------------------------------Private include
#include <includes.h>
//-------------------------------------------------------------------------------Private define
//快速擦除算法使能(待充分测试后,可保留此项,如不稳定,则屏蔽此项)
#define MEMORY_FLASH_FAST_ERASE_ENABLE
//-------------------------------------------------------------------------------Private variables
//系统信号量定义
MODULE_OS_SEM(Sem_EepromFlashUse);
MODULE_OS_SEM(Sem_EepromUse);
MODULE_OS_SEM(Sem_FlashUse);
MODULE_OS_SEM(Sem_FlashModofictionUse);
//用于输入函数修改(后续优化可用内存管理优化掉)
uint8_t MemberBuf[MEMORY_FLASH_MAX_WRITE_SIZE];
S_MEMORY_EEPROM_FLASH* p_sEepromFlash_MemoryEnterFuction;
//用于输出函数(后续优化可用内存管理优化掉)
uint32_t Member_I32;
uint32_t Member_J32;
uint8_t *p_I8;
uint16_t *p_I16;
...
*/
//用于全局通用变量(后续优化可用内存管理优化掉)
uint8_t Member_PrintBuf[100];
//结构缓存
S_MEMORY_VER *p_Ver;
S_CHECK *p_Check;
...
//每个记录/报警的编号不能重,目前用的是伪ms时间来表达.
uint8_t Memory_RecordNum_ms=0;
//-------------------------------------------------------------------------------Private enum
...
//                     入口函数---按地址写数据
//-------------------------------------------------------------------------------
static uint8_t Memory_MachineAddr=0xFF;
static uint8_t Memory_UartBaudRate=0xFF;
static uint32_t Memory_msCount = 0;
uint8_t Memory_EnterFunction(uint8_t cmd,uint32_t addr,uint8_t *buf,uint16_t len)
{
    ...
    if(first==1)
    {
        first=0;
#ifdef  OS_UCOSII
        Sem_EepromFlashUse=OSSemCreate(1);
        Sem_EepromUse=OSSemCreate(1);
        Sem_FlashUse=OSSemCreate(1);
        Sem_FlashModofictionUse=OSSemCreate(1);
#endif
#ifdef  OS_UCOSIII
        OSSemCreate(&Sem_EepromFlashUse     ,"Sem_EepromFlashUse"     ,1,&err);
        OSSemCreate(&Sem_EepromUse          ,"Sem_EepromUse"          ,1,&err);
        OSSemCreate(&Sem_FlashUse           ,"Sem_FlashUse"           ,1,&err);
        OSSemCreate(&Sem_FlashModofictionUse,"Sem_FlashModofictionUse",1,&err);
#endif
    }
    switch(cmd)
    {
...
#endif
#ifdef OS_UCOSIII
            OSSemPend(&Sem_EepromUse,0,OS_OPT_PEND_BLOCKING,NULL,&err);
#endif
            BspEeprom_Init();
#ifdef OS_UCOSII
...
#endif
#ifdef OS_UCOSIII
            OSSemPost(&Sem_EepromUse,OS_OPT_POST_1,&err);
#endif
#else
            I2C_Init();
            break;
        ...
        case MEMORY_ENTRY_CMD_EEPROM_READ:
            ...
            OSSemPend(&Sem_EepromUse,0,OS_OPT_PEND_BLOCKING,NULL,&err);
#endif
            BspEeprom_Read((uint16_t)addr,buf,len);
#ifdef OS_UCOSII
            OSSemPost(Sem_EepromUse);
#endif
#ifdef OS_UCOSIII
        ...
        case MEMORY_ENTRY_CMD_EEPROM_WRITE:
            ...
            while(1)
            {
                BspEeprom_Write((uint16_t)addr,buf,len);
                //校验(如果最后两个字节写正确,则认为整帧数据写正确)
                BspEeprom_Read((uint16_t)addr+len-2,buf1,2);
...
#endif
#ifdef OS_UCOSIII
            OSSemPend(&Sem_FlashUse,0,OS_OPT_PEND_BLOCKING,NULL,&err);
#endif
            Flash_Init();
#ifdef OS_UCOSII
...
#endif
#ifdef OS_UCOSIII
            OSSemPost(&Sem_FlashUse,OS_OPT_POST_1,&err);
#endif
#else
            Spi1MasterInit();
            break;
        ...
        case MEMORY_ENTRY_CMD_FLASH_MODIFICATION:
...
#endif
#ifdef OS_UCOSIII
            OSSemPend(&Sem_FlashModofictionUse,0,OS_OPT_PEND_BLOCKING,NULL,&err);
#endif
            //计算应用区头地址-->i32
            ...
            //获取备份块编号-->i
            {
                ...
                res = Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_EEPROM_FLASH_ADDR,(uint8_t*)p_sEepromFlash_MemoryEnterFuction,sizeof(S_MEMORY_EEPROM_FLASH));
            ...
            {
                p_sEepromFlash_MemoryEnterFuction = (S_MEMORY_EEPROM_FLASH*)MemberBuf;
                memset((uint8_t*)p_sEepromFlash_MemoryEnterFuction,0xFF,sizeof(S_MEMORY_EEPROM_FLASH));
                memcpy((uint8_t*)p_sEepromFlash_MemoryEnterFuction,MEMORY_EEPROM_FLASH_HEAD_STR,6);
                p_sEepromFlash_MemoryEnterFuction->FalshAppAddr=i32;
                p_sEepromFlash_MemoryEnterFuction->SaveBlockNum=i;
                p_sEepromFlash_MemoryEnterFuction->Sum=Count_Sum(0,(uint8_t*)p_sEepromFlash_MemoryEnterFuction,sizeof(S_MEMORY_EEPROM_FLASH)-2);
                res = Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_WRITE,MEMORY_EEPROM_FLASH_ADDR,(uint8_t*)p_sEepromFlash_MemoryEnterFuction,sizeof(S_MEMORY_EEPROM_FLASH));
            }
            if(res==ERR)goto GotoMemoryEnter;
            //擦除应用区域
...
#endif
#ifdef OS_UCOSIII
            OSSemPost(&Sem_FlashModofictionUse,OS_OPT_POST_1,&err);
#endif
            break;
        default:
            break;
...
//-------------------------------------------------------------------------------
static uint8_t Memory_InternalApp(uint16_t Cmd,uint8_t *Buf,uint32_t Number,uint32_t SerialNumber)
{
    uint8_t i;
    uint16_t i16;
    uint32_t i32,j32;
    ...
    //申请缓存
    uint8_t *Member_SBuf;
    Member_SBuf=MemManager_Get(E_MEM_MANAGER_TYPE_256B);
    //
    switch(Cmd)
    {
        //FLASH擦除
        case MEMORY_APP_CMD_FLASH_EARSE:
            Memory_EnterFunction(MEMORY_ENTRY_CMD_FALSH_MANY_SECTOR_ERASE,Number,Buf,SerialNumber);
            break;
        //FLASH写入
        case  MEMORY_APP_CMD_FLASH_WRITE:
            Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_WRITE,Number,Buf,SerialNumber);
            break;
        //FLASH读取
        case MEMORY_APP_CMD_FLASH_READ:
            Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_READ,Number,Buf,SerialNumber);
            break;
        //铁电初始化测试
        ...
        case MEMORY_APP_CMD_VER_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_VER)-2);
        ...
        {
            p_Ver_MemoryApp = (S_MEMORY_VER*)Buf;
            p_Ver_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_VER)-2);
        ...
        case MEMORY_APP_CMD_KEEPPARA_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_KEEPPARA)-2);
        ...
        {
            p_KeepPara_MemoryApp = (S_MEMORY_KEEPPARA*)Buf;
            p_KeepPara_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_KEEPPARA)-2);
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_WRITE,MEMORY_KEEPPARA_ADDR,Buf,sizeof(S_MEMORY_KEEPPARA));
            break;
        }
        //读网络参数
        case MEMORY_APP_CMD_NET_R:
        {
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_NET_ADDR,Buf,sizeof(S_MEMORY_NET));
            if(res==ERR)
            {
                res = MEMORY_APP_ERR_OPERATION;
                break;
            }
            p_Net_MemoryApp = (S_MEMORY_NET*)Buf;
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_NET)-2);
            if(i16==p_Net_MemoryApp->Sum)
            {
                res=MEMORY_APP_ERR_NO;
                break;
            }
        ...
        //写网络参数
        case MEMORY_APP_CMD_NET_W:
        {
            p_Net_MemoryApp = (S_MEMORY_NET*)Buf;
            p_Net_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_NET)-2);
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_WRITE,MEMORY_NET_ADDR,Buf,sizeof(S_MEMORY_NET));
            break;
        }
        //读控制
        case MEMORY_APP_CMD_CONTROL_R:
        {
            ...
            i16=Count_Sum(0,Buf,sizeof(S_MEMORY_CONTROL)-2);
        ...
        {
            p_Control_MemoryApp = (S_MEMORY_CONTROL*)Buf;
            p_Control_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_CONTROL)-2);
        ...
        case MEMORY_APP_CMD_DOOR_R:
        {
            ...
            i16=Count_Sum(0,Buf,sizeof(S_MEMORY_DOOR)-2);
        ...
        case MEMORY_APP_CMD_DOOR_W:
        {
            ...
            {
                p_Door_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_DOOR)-2);
            }
            if(Number>=MEMORY_DOOR_MAX_NUM)
            {
                res = MEMORY_APP_ERR_PARA;
            ...
            if(res==MEMORY_APP_ERR_NO)
            {
                ...
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_DOOR,0);
                if(res==MEMORY_APP_ERR_NO)
                {
                    //流水号
                    ...
                    //校验指针
                    p_I16=(uint16_t*)&Member_I32;
                    *p_I16=0;
                    for(count=i=0; i<MEMORY_DOOR_MAX_NUM; i++)
                    {
                        res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_DOOR_ADDR+i*MEMORY_DOOR_SIZE+sizeof(S_MEMORY_DOOR)-2,(uint8_t *)p_I16,2);
                        if(res!=OK)break;
                    ...
                    if(res!=OK)break;
                    res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_DOOR,0);
                }
            }
            break;
        }
        ...
        */
        ...
        case MEMORY_APP_CMD_CARDGBITMAP_R:
        {
            ...
            i16=Count_Sum(0,Buf,sizeof(S_MEMORY_CARDG_BITMAP)-2);
        ...
        case MEMORY_APP_CMD_CARDGBITMAP_W:
        {
            ...
            {
                p_CardGBitmap_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_CARDG_BITMAP)-2);
            }
            if(Number>=MEMORY_CARDGBITMAP_MAX_NUM)
            {
                res = MEMORY_APP_ERR_PARA;
            ...
            {
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARDG_BITMAP,0);
                if(res==MEMORY_APP_ERR_NO)
                {
                    //流水号
                    p_Check_MemoryApp->SerialNumber=SerialNumber;
                    ...
                    *p_I16=0;
                    ...
                    {
                        res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_CARDGBITMAP_ADDR+i*MEMORY_CARDGBITMAP_SIZE+sizeof(S_MEMORY_CARDG_BITMAP)-2,(uint8_t*)p_I16,2);
                        if(res!=OK)break;
                        if((*p_I16!=0xFFFF)&&(*p_I16!=0))
                        {
                            count++;
                    ...
                    p_Check_MemoryApp->Number=count;
                    if(res!=OK)break;
                    res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARDG_BITMAP,0);
                }
            }
            break;
        }
        ...
        case MEMORY_APP_CMD_READER_R:
        {
            ...
            i16=Count_Sum(0,Buf,sizeof(S_MEMORY_READER)-2);
        ...
        case MEMORY_APP_CMD_READER_W:
        {
            ...
            {
                p_Reader_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_READER)-2);
            }
            if(Number>=MEMORY_READER_MAX_NUM)
            {
                res = MEMORY_APP_ERR_PARA;
            ...
            {
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_READER,0);
                if(res==MEMORY_APP_ERR_NO)
                {
                    //流水号
                    p_Check_MemoryApp->SerialNumber=SerialNumber;
                    ...
                    *p_I16=0;
                    ...
                    {
                        res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_READER_ADDR+i*MEMORY_READER_SIZE+sizeof(S_MEMORY_READER)-2,(uint8_t*)p_I16,2);
                        if(res!=OK)break;
                        if((*p_I16!=0xFFFF)&&(*p_I16!=0))
                        {
                            count++;
                    ...
                    p_Check_MemoryApp->Number=count;
                    if(res!=OK)break;
                    res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_READER,0);
                }
            }
            break;
        }
        ...
        case MEMORY_APP_CMD_RELAY_R:
        {
            ...
            i16=Count_Sum(0,Buf,sizeof(S_MEMORY_RELAY)-2);
        ...
        case MEMORY_APP_CMD_RELAY_W:
        {
            ...
            {
                p_Relay_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_RELAY)-2);
            }
            if(Number>=MEMORY_RELAY_MAX_NUM)
            {
                res = MEMORY_APP_ERR_PARA;
            ...
            {
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_RELAY,0);
                if(res==MEMORY_APP_ERR_NO)
                {
                    //流水号
                    p_Check_MemoryApp->SerialNumber=SerialNumber;
                    ...
                    *p_I16=0;
                    ...
                    {
                        res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_RELAY_ADDR+i*MEMORY_RELAY_SIZE+sizeof(S_MEMORY_RELAY)-2,(uint8_t*)p_I16,2);
                        if(res!=OK)break;
                        if((*p_I16!=0xFFFF)&&(*p_I16!=0))
                        {
                            count++;
                    ...
                    p_Check_MemoryApp->Number=count;
                    if(res!=OK)break;
                    res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_RELAY,0);
                }
            }
            break;
        }
        ...
        case MEMORY_APP_CMD_INPUT_R:
        {
            ...
            i16=Count_Sum(0,Buf,sizeof(S_MEMORY_INPUT)-2);
        ...
        case MEMORY_APP_CMD_INPUT_W:
        {
            ...
            {
                p_Input_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_INPUT)-2);
            }
            if(Number>=MEMORY_INPUT_MAX_NUM)
            {
                res = MEMORY_APP_ERR_PARA;
            ...
            {
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_INPUT,0);
                if(res==MEMORY_APP_ERR_NO)
                {
                    //流水号
                    p_Check_MemoryApp->SerialNumber=SerialNumber;
                    ...
                    *p_I16=0;
                    ...
                    {
                        res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_INPUT_ADDR+i*MEMORY_INPUT_SIZE+sizeof(S_MEMORY_INPUT)-2,(uint8_t*)p_I16,2);
                        if(res!=OK)break;
                        if((*p_I16!=0xFFFF)&&(*p_I16!=0))
                        {
                            count++;
                    ...
                    p_Check_MemoryApp->Number=count;
                    if(res!=OK)break;
                    res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_INPUT,0);
                }
            }
            break;
        }
        ...
        case MEMORY_APP_CMD_ALARM_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_ALARM)-2);
        ...
        case MEMORY_APP_CMD_ALARM_W:
        {
            p_Alarm_MemoryApp = (S_MEMORY_ALARM*)Buf;
            //如果为删除指令,校验固定写为0xFFFF
            p_Alarm_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_ALARM)-2);
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_WRITE,MEMORY_ALARM_ADDR,Buf,sizeof(S_MEMORY_ALARM));
            if(res==MEMORY_APP_ERR_NO)
            {
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_ALARM,0);
                if(res==MEMORY_APP_ERR_NO)
                {
                    //流水号
                    p_Check_MemoryApp->SerialNumber=SerialNumber;
                    ...
                    *p_I16=0;
                    ...
                    for(count=i=0; i<1; i++)
                    {
                        res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_ALARM_ADDR+i*MEMORY_ALARM_SIZE+sizeof(S_MEMORY_ALARM)-2,(uint8_t*)p_I16,2);
                        if(res!=OK)break;
                        if((*p_I16!=0xFFFF)&&(*p_I16!=0))
                        {
                            count++;
                    ...
                    p_Check_MemoryApp->Number=count;
                    if(res!=OK)break;
                    res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_ALARM,0);
                }
            }
            break;
        }
        ...
        case MEMORY_APP_CMD_DOORGROUP_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_DOOR_GROUP)-2);
        ...
        case MEMORY_APP_CMD_DOORGROUP_W:
        {
            ...
            {
                p_DoorG_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_DOOR_GROUP)-2);
            }
            //修正总校验和
            p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
            //校验指针
            p_I16=(uint16_t*)&Member_I32;
            *p_I16=0;
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_DOORGROUP,0);
            if(res!=OK)break;
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_READ ,MEMORY_DOORGROUP_ADDR+Number*MEMORY_DOORGROUP_SIZE+sizeof(S_MEMORY_DOOR_GROUP)-2,(uint8_t*)p_I16,2);
            ...
            //写入校验和
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_DOORGROUP,0);
            break;
        }
        //读卡组
        case MEMORY_APP_CMD_CARDGROUP_LIMIT_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_CARDGROUP_LIMIT)-2);
        ...
        case MEMORY_APP_CMD_CARDGROUP_LIMIT_W:
        {
            ...
            {
                p_CardG_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_CARDGROUP_LIMIT)-2);
            }
            //修正总校验和
            p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
            //校验指针
            ...
            *p_I16=0;
            //
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARDGROUP,0);
            if(res!=OK)break;
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_READ ,MEMORY_CARDGROUPLIMIT_ADDR+Number*MEMORY_CARDGROUPLIMIT_SIZE+sizeof(S_MEMORY_CARDGROUP_LIMIT)-2,(uint8_t*)p_I16,2);
            ...
            //写入校验和
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARDGROUP,0);
            break;
        }
        //读区域
        case MEMORY_APP_CMD_AREA_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_AREA)-2);
        ...
        case MEMORY_APP_CMD_AREA_W:
        {
            ...
            {
                p_Area_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_AREA)-2);
            }
            //修正总校验和
            p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
            //校验指针
            ...
            *p_I16=0;
            //
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_AREA,0);
            if(res!=OK)break;
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_READ ,MEMORY_AREA_ADDR+Number*MEMORY_AREA_SIZE+sizeof(S_MEMORY_AREA)-2,(uint8_t*)p_I16,2);
            ...
            //写入校验和
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_AREA,0);
            break;
        }
        //读时间组
        case MEMORY_APP_CMD_TIMEGROUP_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_TIMEGROUP)-2);
        ...
        case MEMORY_APP_CMD_TIMEGROUP_W:
        {
            ...
            {
                p_TimeG_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_TIMEGROUP)-2);
            }
            //修正总校验和
            p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
            //校验指针
            ...
            *p_I16=0;
            //
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_TIMEGROUP,0);
            if(res!=OK)break;
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_READ ,MEMORY_FLASH_TIMEGROUP_ADDR+Number*MEMORY_FLASH_TIMEGROUP_SIZE+sizeof(S_MEMORY_TIMEGROUP)-2,(uint8_t*)p_I16,2);
            ...
            //写入校验和
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_TIMEGROUP,0);
            break;
        }
        //读节假日组
        case MEMORY_APP_CMD_HOLIDAYGROUP_R:
        {
            ...
            i16 = Count_Sum(0,Buf,sizeof(S_MEMORY_HOLIDAYGROUP)-2);
        ...
        case MEMORY_APP_CMD_HOLIDAYGROUP_W:
        {
            ...
            {
                p_HolidayG_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_HOLIDAYGROUP)-2);
            }
            //修正总校验和
            p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
            //校验指针
            ...
            *p_I16=0;
            //
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_HOLIDAYGROUP,0);
            if(res!=OK)break;
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_READ ,MEMORY_FLASH_HOLIDAYGROUP_ADDR+Number*MEMORY_FLASH_HOLIDAYGROUP_SIZE+sizeof(S_MEMORY_HOLIDAYGROUP)-2,(uint8_t*)p_I16,2);
            ...
            //写入校验和
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_HOLIDAYGROUP,0);
            break;
        }
        //读用户权限
        case MEMORY_APP_CMD_USER_PERMISSION_R:
            ...
            if(*p_i32!=0)
            {
                ...
                //---获取当前记录数量(s_check)
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER1+i,0);
                ...
                p_i32=(uint32_t*)Buf;
                //---获取当前记录数量(s_check)
                res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_VISITOR,0);
                if(p_Check_MemoryApp->Number>MEMORY_FLASH_USER_PERMISSION_MAX_NUM)
                {
                    res=MEMORY_APP_ERR_PARA;
                    break;
        ...
        case MEMORY_APP_CMD_NEW_USER_W:
        {
            ...
            //本条校验和计算
            p_User_MemoryApp=(S_MEMORY_USER_PERMISSION*)Buf;
            p_User_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_USER_PERMISSION)-2);
            //提取卡号(*p_i32)
            i32=Buf[0];
            i32<<=8;
            i32+=Buf[1];
            ...
            p_Check_MemoryApp->SerialNumber=SerialNumber;
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER1+i,0);
            break;
        }
        //写访客权限(新建)
        case MEMORY_APP_CMD_NEW_VISITOR_W:
        {
            ...
            p_Check_MemoryApp->CheckSum+=p_User_MemoryApp->Sum;
            p_Check_MemoryApp->SerialNumber=SerialNumber;
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_VISITOR,0);
            break;
        }
        //修改普通用户
        ...
        case MEMORY_APP_CMD_CARD_RECORD_W:
            ...
            *p_I16=0;
            //获取当前记录数量
            res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_I16,MEMORY_APP_CHECK_CARD_RECORD,0);
            if(res==ERR)break;
            //查看是否越界
            ...
            (*p_I16)++;
            res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_I16,MEMORY_APP_CHECK_CARD_RECORD,0);
            break;
        //读报警记录
        ...
        case MEMORY_APP_CMD_ALARM_RECORD_W:
            ...
            *p_I16=0;
            //获取当前记录数量
            res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_I16,MEMORY_APP_CHECK_ALARM_RECORD,0);
            if(res==ERR)break;
            //查看是否越界
            ...
            (*p_I16)++;
            res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_I16,MEMORY_APP_CHECK_ALARM_RECORD,0);
            break;
        //读输入输出与模块
        case  MEMORY_APP_CMD_LOGICBLOCK_INOUTAND_R:
            ...
            i16=Count_Sum(0,Buf,sizeof(S_MEMORY_LOGICBLOCK_INOUTAND)-2);
        ...
        case  MEMORY_APP_CMD_LOGICBLOCK_INOUTAND_W:
            ...
            {
                p_LogicBlockInOutAnd_MemoryApp->Sum = Count_Sum(0,Buf,sizeof(S_MEMORY_LOGICBLOCK_INOUTAND)-2);
            }
            //修正总校验和
            p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
            //校验指针
            ...
            *p_I16=0;
            //
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_LOGICBLOCK_INOUTAND,0);
            if(res!=OK)break;
            res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_LOGICBLOCK_INOUTAND_ADDR+Number*MEMORY_LOGICBLOCK_INOUTAND_SIZE+sizeof(S_MEMORY_LOGICBLOCK_INOUTAND)-2,(uint8_t *)p_I16,2);
            ...
            //写入校验和
            res=Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_LOGICBLOCK_INOUTAND,0);
            break;
        /*
        //读逻辑模块之输入模块
        ...
        */
        ...
        case MEMORY_APP_CMD_FORMAT:
            ...
            if(Number==MEMORY_FORMAT_ALL)
            {
                //处理不随初始化更改的数据
                //清EEPROM
                for(i32=0; i32<MEMORY_EERPOM_END_ADDR; i32+=256)
                {
                ...
                if(res!=MEMORY_APP_ERR_NO)break;
                //写EEPROM头
                res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_WRITE,MEMORY_EEPROM_HEAD_ADDR,(uint8_t*)MEMORY_PRODUCT_HEAD,strlen(MEMORY_PRODUCT_HEAD));
                if(res==ERR)
                {
                    res = MEMORY_APP_ERR_OPERATION;
                    break;
                ...
                //写版本
                {
                    ...
                    memset((uint8_t*)p_Ver_MemoryApp,0,sizeof(S_MEMORY_VER));
                    ...
                    p_Ver_MemoryApp->Sum=Count_Sum(0,(uint8_t*)p_Ver_MemoryApp,sizeof(S_MEMORY_VER)-2);
                    res = Memory_InternalApp(MEMORY_APP_CMD_VER_W,(uint8_t*)p_Ver_MemoryApp,0,0);
                    if(res!=MEMORY_APP_ERR_NO)
                    {
                        break;
                    }
                ...
                //初始化保存数据区
                {
                    ...
                    memset((uint8_t*)p_KeepPara_MemoryApp,0xFF,sizeof(S_MEMORY_KEEPPARA));
                    ...
                    p_KeepPara_MemoryApp->Sum=Count_Sum(0,(uint8_t*)p_KeepPara_MemoryApp,sizeof(S_MEMORY_KEEPPARA)-2);
                    res = Memory_InternalApp(MEMORY_APP_CMD_KEEPPARA_W,(uint8_t*)p_KeepPara_MemoryApp,0,0);
                    if(res!=MEMORY_APP_ERR_NO)
                    {
                        break;
                    }
                ...
                //初始化网络参数
                {
                    p_Net_MemoryApp=(S_MEMORY_NET*)Member_SBuf;
                    //
                    Memory_InternalApp(MEMORY_APP_CMD_NET_R,(uint8_t*)p_Net_MemoryApp,0,0);
                    if(p_Net_MemoryApp->Gateway_IP[0]==0xFF
                       &&p_Net_MemoryApp->Gateway_IP[1]==0xFF
                       &&p_Net_MemoryApp->Gateway_IP[2]==0xFF
                       &&p_Net_MemoryApp->Gateway_IP[3]==0xFF
                      )
                    {
                        memset((uint8_t*)p_Net_MemoryApp,0xFF,sizeof(S_MEMORY_NET));
                        //
                        p_Net_MemoryApp->Gateway_IP[0] =  192;
                        p_Net_MemoryApp->Gateway_IP[1] =  168;
                        p_Net_MemoryApp->Gateway_IP[2] =  1;
                        p_Net_MemoryApp->Gateway_IP[3] =  1;
                        p_Net_MemoryApp->Sub_Mask[0]   =  255;
                        p_Net_MemoryApp->Sub_Mask[1]   =  255;
                        p_Net_MemoryApp->Sub_Mask[2]   =  255;
                        p_Net_MemoryApp->Sub_Mask[3]   =  0;
                        p_Net_MemoryApp->IP_Local[0]   =  192;
                        p_Net_MemoryApp->IP_Local[1]   =  168;
                        p_Net_MemoryApp->IP_Local[2]   =  1;
                        p_Net_MemoryApp->IP_Local[3]   =  241;
                        p_Net_MemoryApp->Phy_Addr[0]   =  0x0c;
                        p_Net_MemoryApp->Phy_Addr[1]   =  0x29;
                        p_Net_MemoryApp->Phy_Addr[2]   =  p_Net_MemoryApp->IP_Local[0];
                        p_Net_MemoryApp->Phy_Addr[3]   =  p_Net_MemoryApp->IP_Local[1];
                        p_Net_MemoryApp->Phy_Addr[4]   =  p_Net_MemoryApp->IP_Local[2];
                        p_Net_MemoryApp->Phy_Addr[5]   =  p_Net_MemoryApp->IP_Local[3];
                        p_Net_MemoryApp->DNS_IP[0]     =  8;
                        p_Net_MemoryApp->DNS_IP[1]     =  8;
                        p_Net_MemoryApp->DNS_IP[2]     =  8;
                        p_Net_MemoryApp->DNS_IP[3]     =  8;
                        p_Net_MemoryApp->IP_Dest1[0]   =  192;
                        p_Net_MemoryApp->IP_Dest1[1]   =  168;
                        p_Net_MemoryApp->IP_Dest1[2]   =  1;
                        p_Net_MemoryApp->IP_Dest1[3]   =  210;
                        p_Net_MemoryApp->PORT_Dest1    =  9600;
                        //
                        p_Net_MemoryApp->Sum=Count_Sum(0,(uint8_t*)p_Net_MemoryApp,sizeof(S_MEMORY_NET)-2);
                        res = Memory_InternalApp(MEMORY_APP_CMD_NET_W,(uint8_t*)p_Net_MemoryApp,0,0);
                        if(res!=MEMORY_APP_ERR_NO)
                        {
                            break;
                        }
                ...
                //写校验
                {
                    ...
                    memset((uint8_t*)p_MemoryCheck_MemoryApp,0x00,sizeof(S_MEMORY_CHECK));
                    p_MemoryCheck_MemoryApp->Sum=Count_Sum(0,(uint8_t*)p_MemoryCheck_MemoryApp,sizeof(S_MEMORY_CHECK)-2);
                    res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_MemoryCheck_MemoryApp,MEMORY_APP_CHECK_ALL,0);
                    if(res!=MEMORY_APP_ERR_NO)
                    {
                        break;
                    }
                ...
                //写控制
                {
                    ...
                    memset((uint8_t*)p_Control_MemoryApp,0x00,sizeof(S_MEMORY_CONTROL));
                    p_Control_MemoryApp->DeviceEnable=ENABLE;
                    p_Control_MemoryApp->Sum=Count_Sum(0,(uint8_t*)p_Control_MemoryApp,sizeof(S_MEMORY_CONTROL)-2);
                    res = Memory_InternalApp(MEMORY_APP_CMD_CONTROL_W,(uint8_t*)p_Control_MemoryApp,0,0);
                    if(res!=MEMORY_APP_ERR_NO)
                    {
                        break;
                    }
                ...
                //写信息
                {
                    res=Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_WRITE,0,(uint8_t*)MEMORY_PRODUCT_HEAD,strlen(MEMORY_PRODUCT_HEAD));
                    if(res==ERR)
                    {
                        res = MEMORY_APP_ERR_OPERATION;
                        break;
            ...
            if(Number&MEMORY_FORMAT_ALARM_RECORD)
            {
                ...
                p_I16=(uint16_t*)&Member_I32;
                *p_I16=0;
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_I16,MEMORY_APP_CHECK_ALARM_RECORD,0);
            }
            if(Number&MEMORY_FORMAT_CARD_RECORD)
            {
                ...
                p_I16=(uint16_t*)&Member_I32;
                *p_I16=0;
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_I16,MEMORY_APP_CHECK_CARD_RECORD,0);
            }
            if(Number&MEMORY_FORMAT_VISITOR)
            {
                ...
                //清校验
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_VISITOR,0);
            }
            if(Number&MEMORY_FORMAT_USER1)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER1,0);
            }
            if(Number&MEMORY_FORMAT_USER2)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER2,0);
            }
            if(Number&MEMORY_FORMAT_USER3)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER3,0);
            }
            if(Number&MEMORY_FORMAT_USER4)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER4,0);
            }
            if(Number&MEMORY_FORMAT_USER5)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER5,0);
            }
            if(Number&MEMORY_FORMAT_USER6)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER6,0);
            }
            if(Number&MEMORY_FORMAT_USER7)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER7,0);
            }
            if(Number&MEMORY_FORMAT_USER8)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER8,0);
            }
            if(Number&MEMORY_FORMAT_USER9)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER9,0);
            }
            if(Number&MEMORY_FORMAT_USER10)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER10,0);
            }
            if(Number&MEMORY_FORMAT_USER11)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER11,0);
            }
            if(Number&MEMORY_FORMAT_USER12)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER12,0);
            }
            if(Number&MEMORY_FORMAT_USER13)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER13,0);
            }
            if(Number&MEMORY_FORMAT_USER14)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER14,0);
            }
            if(Number&MEMORY_FORMAT_USER15)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER15,0);
            }
            if(Number&MEMORY_FORMAT_USER16)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARD_USER16,0);
            }
            if(Number&MEMORY_FORMAT_HOLIDAYG)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_HOLIDAYGROUP,0);
            }
            if(Number&MEMORY_FORMAT_TIMEG)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_TIMEGROUP,0);
            }
            if(Number&MEMORY_FORMAT_DOOR)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_DOOR,0);
            }
            if(Number&MEMORY_FORMAT_DOORG)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_DOORGROUP,0);
            }
            if(Number&MEMORY_FORMAT_CARDG)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARDGROUP,0);
            }
            if(Number&MEMORY_FORMAT_AREA)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_AREA,0);
            }
            /*
            ...
            */
            if(Number&MEMORY_FORMAT_CARDGB)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_CARDG_BITMAP,0);
            }
            if(Number&MEMORY_FORMAT_READER)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_READER,0);
            }
            if(Number&MEMORY_FORMAT_RELAY)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_RELAY,0);
            }
            if(Number&MEMORY_FORMAT_INPUT)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_INPUT,0);
            }
            if(Number&MEMORY_FORMAT_INOUTAND)
            {
                ...
                p_Check_MemoryApp=(S_CHECK*)Member_SBuf;
                memset((uint8_t*)p_Check_MemoryApp,0x00,sizeof(S_CHECK));
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_Check_MemoryApp,MEMORY_APP_CHECK_LOGICBLOCK_INOUTAND,0);
            }
            /*
        ...
        case MEMORY_APP_CMD_INIT:
            ...
            //读取铁电和FLASH,查看是否有效,如果无效,则进行格式化
            {
                ...
                res = Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_EEPROM_HEAD_ADDR,(uint8_t*)Member_PrintBuf,strlen(MEMORY_PRODUCT_HEAD));
                ...
                //验证FLASH头
                memset(Member_PrintBuf,0,10);
                res=Memory_EnterFunction(MEMORY_ENTRY_CMD_FLASH_READ,0,(uint8_t*)Member_PrintBuf,strlen(MEMORY_PRODUCT_HEAD));
                if(0!=strcmp((const char*)Member_PrintBuf,MEMORY_PRODUCT_HEAD))
                {
                    res = Memory_InternalApp(MEMORY_APP_CMD_FORMAT,0,MEMORY_FORMAT_ALL,0);
                    res = MEMORY_APP_ERR_FIRST;
            ...
            {
                p_KeepPara_MemoryApp = (S_MEMORY_KEEPPARA*)Member_SBuf;
                Memory_InternalApp(MEMORY_APP_CMD_KEEPPARA_R,(uint8_t*)p_KeepPara_MemoryApp,0,0);
                ...
                Memory_InternalApp(MEMORY_APP_CMD_KEEPPARA_W,(uint8_t*)p_KeepPara_MemoryApp,0,0);
            }
            //读取备份区,看是否有数据需要恢复
            //读EEPROM标志-->addr
            {
                p_EepromFlash_MemoryApp = (S_MEMORY_EEPROM_FLASH*)Member_SBuf;
                res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_READ,MEMORY_EEPROM_FLASH_ADDR,(uint8_t*)p_EepromFlash_MemoryApp,sizeof(S_MEMORY_EEPROM_FLASH));
                if(res==ERR)break;
                ...
                i16=Count_Sum(0,(uint8_t*)p_EepromFlash_MemoryApp,sizeof(S_MEMORY_EEPROM_FLASH)-2);
                ...
                //清EEPROM
                memset((uint8_t*)p_EepromFlash_MemoryApp,0xFF,sizeof(S_MEMORY_EEPROM_FLASH));
                res=Memory_EnterFunction(MEMORY_ENTRY_CMD_EEPROM_WRITE,MEMORY_EEPROM_FLASH_ADDR,(uint8_t*)p_EepromFlash_MemoryApp,sizeof(S_MEMORY_EEPROM_FLASH));
                if(res==ERR)break;
            }
            break;
        default:
    ...
    //释放缓存
    MemManager_Free(E_MEM_MANAGER_TYPE_256B,Member_SBuf);
    //
    return res;
}
//-------------------------------------------------------------------------------
//函数功能: 将内部函数封装为外部函
//-------------------------------------------------------------------------------
uint8_t Memory_AppFunction(uint16_t Cmd,uint8_t *Buf,uint32_t Number,uint32_t SerialNumber)
{
    uint8_t res;
...
//-------------------------------------------------------------------------------
void Memory_TEST(uint32_t cmd)
{
    uint8_t sign_exe=FALSE;
    uint8_t res;
    uint8_t i;
    uint8_t *p;
    uint16_t i16,j16,k16,m16;
    uint32_t i32,j32;
    uint32_t *MemberTestBuf;
#ifdef OS_UCOSII
    uint8_t err;
...
#ifdef OS_UCOSIII
    OS_ERR err;
    OSSemPend(&Sem_EepromFlashUse,0,OS_OPT_PEND_BLOCKING,NULL,&err);
#endif
    //申请缓存
    MemberTestBuf = MemManager_Get(E_MEM_MANAGER_TYPE_256B);
    //-----地址空间输出
    if(cmd & Memory_TEST_READ_BASEINFO)
    {
        //
        sprintf((char*)Member_PrintBuf,"存储器地址分配:\r\n");
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        //
        ...
        //
        sprintf((char*)Member_PrintBuf,"基本信息:\r\n");
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Ver = (S_MEMORY_VER*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_VER_R,(uint8_t*)p_Ver,0,0);
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        //
        sprintf((char*)Member_PrintBuf,"网络信息:\r\n");
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Net = (S_MEMORY_NET*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_NET_R,(uint8_t*)p_Net,0,0);
        sprintf((char*)Member_PrintBuf,"  网络网关: %03d.%03d.%03d.%03d\r\n",p_Net->Gateway_IP[0],p_Net->Gateway_IP[1],p_Net->Gateway_IP[2],p_Net->Gateway_IP[3]);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        sprintf((char*)Member_PrintBuf,"  子网掩码: %03d.%03d.%03d.%03d\r\n",p_Net->Sub_Mask[0],p_Net->Sub_Mask[1],p_Net->Sub_Mask[2],p_Net->Sub_Mask[3]);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        sprintf((char*)Member_PrintBuf,"  本机IP  : %03d.%03d.%03d.%03d\r\n",p_Net->IP_Local[0],p_Net->IP_Local[1],p_Net->IP_Local[2],p_Net->IP_Local[3]);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        sprintf((char*)Member_PrintBuf,"  物理地址: %02x:%02x:%02x:%02x:%02x:%02x\r\n",p_Net->Phy_Addr[0],p_Net->Phy_Addr[1],p_Net->Phy_Addr[2],p_Net->Phy_Addr[3],p_Net->Phy_Addr[4],p_Net->Phy_Addr[5]);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        sprintf((char*)Member_PrintBuf,"  DNS     : %03d.%03d.%03d.%03d\r\n",p_Net->DNS_IP[0],p_Net->DNS_IP[1],p_Net->DNS_IP[2],p_Net->DNS_IP[3]);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        sprintf((char*)Member_PrintBuf,"  目标1   : %03d.%03d.%03d.%03d:%05d\r\n",p_Net->IP_Dest1[0],p_Net->IP_Dest1[1],p_Net->IP_Dest1[2],p_Net->IP_Dest1[3],p_Net->PORT_Dest1);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        //
        sprintf((char*)Member_PrintBuf,"其他信息:\r\n");
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Control = (S_MEMORY_CONTROL*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_CONTROL_R,(uint8_t*)p_Control,0,0);
        ...
        Memory_InternalApp(MEMORY_APP_CMD_KEEPPARA_R,(uint8_t*)p_KeepPara,0,0);
    ...
    if(cmd & Memory_TEST_READ_CHECK)
    {
        ...
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_MemoryCheck,MEMORY_APP_CHECK_ALL,0);
    ...
    if(cmd & Memory_TEST_READ_DOORG)
    {
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_DOORGROUP_R,(uint8_t*)p_DoorG,k16,0);
        ...
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_DOORGROUP,0);
        sprintf((char*)Member_PrintBuf,"  获取:数量-%05d,校验-%05d,流水-%08ld\r\n",p_Check->Number,p_Check->CheckSum,p_Check->SerialNumber);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
    }
    //-----只读输入输出与
    if(cmd & Memory_TEST_READ_INOUTAND)
    {
        i16=j16=0;
        p_LogicBlockInOutAnd = (S_MEMORY_LOGICBLOCK_INOUTAND*)MemberTestBuf;
        for(k16=0; k16<MEMORY_LOGICBLOCK_INOUTAND_MAX_NUM; k16++)
        {
            Memory_InternalApp(MEMORY_APP_CMD_LOGICBLOCK_INOUTAND_R,(uint8_t*)p_LogicBlockInOutAnd,k16,0);
            if(p_LogicBlockInOutAnd->Sum!=0xFFFF)
            {
                i16++;
                j16+=p_LogicBlockInOutAnd->Sum;
                m16 =p_LogicBlockInOutAnd->Number;
                //
                sprintf((char*)Member_PrintBuf,"  逻辑%03d: 编号-%03d 输入[%d,%d][%d,%d][%d,%d][%d,%d]输出[%d,%d][%d,%d][%d,%d][%d,%d]ID[%d],校验-%05d\r\n",\
                        k16+1,
                        m16,
                        p_LogicBlockInOutAnd->Input1Type,
                        p_LogicBlockInOutAnd->Input1Time_100ms,
                        p_LogicBlockInOutAnd->Input2Type,
                        p_LogicBlockInOutAnd->Input2Time_100ms,
                        p_LogicBlockInOutAnd->Input3Type,
                        p_LogicBlockInOutAnd->Input3Time_100ms,
                        p_LogicBlockInOutAnd->Input4Type,
                        p_LogicBlockInOutAnd->Input4Time_100ms,
                        p_LogicBlockInOutAnd->Output1OnTimeS[0]*256l+p_LogicBlockInOutAnd->Output1OnTimeS[1],
                        p_LogicBlockInOutAnd->Output1_TimeG,
                        p_LogicBlockInOutAnd->Output2OnTimeS[0]*256l+p_LogicBlockInOutAnd->Output2OnTimeS[1],
                        p_LogicBlockInOutAnd->Output2_TimeG,
                        p_LogicBlockInOutAnd->Output3OnTimeS[0]*256l+p_LogicBlockInOutAnd->Output3OnTimeS[1],
                        p_LogicBlockInOutAnd->Output3_TimeG,
                        p_LogicBlockInOutAnd->Output4OnTimeS[0]*256l+p_LogicBlockInOutAnd->Output4OnTimeS[1],
                        p_LogicBlockInOutAnd->Output4_TimeG,
                        p_LogicBlockInOutAnd->AlarmID[0]*256l+p_LogicBlockInOutAnd->AlarmID[1],
                        p_LogicBlockInOutAnd->Sum);
                DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
            }
        }
        //
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Check = (S_CHECK*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_LOGICBLOCK_INOUTAND,0);
        sprintf((char*)Member_PrintBuf,"  获取:数量-%05d,校验-%05d,流水-%08ld\r\n",p_Check->Number,p_Check->CheckSum,p_Check->SerialNumber);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
    }
    //-----只读卡组信息
    if(cmd & Memory_TEST_READ_CARDG)
    {
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CARDGROUP_LIMIT_R,(uint8_t*)p_CardG,k16,0);
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Check = (S_CHECK*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_CARDGROUP,0);
        sprintf((char*)Member_PrintBuf,"  获取:数量-%05d,校验-%05d,流水-%08ld\r\n",p_Check->Number,p_Check->CheckSum,p_Check->SerialNumber);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
    }
    //-----只读区域信息
    if(cmd & Memory_TEST_READ_AREA)
    {
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_AREA_R,(uint8_t*)p_Area,k16,0);
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Check = (S_CHECK*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_AREA,0);
        sprintf((char*)Member_PrintBuf,"  获取:数量-%05d,校验-%05d,流水-%08ld\r\n",p_Check->Number,p_Check->CheckSum,p_Check->SerialNumber);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
    }
    //-----只读节假日组
    if(cmd & Memory_TEST_READ_HOLIDAYGROUP)
    {
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_HOLIDAYGROUP_R,(uint8_t*)p_HolidayG,k16,0);
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Check = (S_CHECK*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_HOLIDAYGROUP,0);
        sprintf((char*)Member_PrintBuf,"  获取:数量-%05d,校验-%05d,流水-%08ld\r\n",p_Check->Number,p_Check->CheckSum,p_Check->SerialNumber);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
    }
    //-----只读时间组
    if(cmd & Memory_TEST_READ_TIMEGROUP)
    {
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_TIMEGROUP_R,(uint8_t*)p_TimeG,k16,0);
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        p_Check = (S_CHECK*)MemberTestBuf;
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_TIMEGROUP,0);
        sprintf((char*)Member_PrintBuf,"  获取:数量-%05d,校验-%05d,流水-%08ld\r\n",p_Check->Number,p_Check->CheckSum,p_Check->SerialNumber);
        DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
    }
    //-----只读用户
    if(cmd & Memory_TEST_READ_USER)
    {
        for(i=0; i<MEMORY_FLASH_USER_PERMISSION_GROUP_NUM; i++)
        {
            ...
            for(k16=0; ; k16++)
            {
                ...
                Memory_InternalApp(MEMORY_APP_CMD_USER_PERMISSION_R,(uint8_t*)p_User,k16,i);
                if(p_User->Sum!=0xFFFF)
                {
                    ...
                    sprintf((char*)Member_PrintBuf,"  序号%03d: 卡号-%05ld,卡组-%05d,密码-0x%02x%02x%02x%02x,卡片状态[%02x],卡类型[%02x],功能选项[%02x%02x],通行等级[%02x],胁迫等级[%02x],校验-%05d\r\n",
                            ...
                            p_User->Sum);
                    DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
                    Count_TimeTypeConvert(2,&Memory_tm,p_User->EffectBeginTime);
                    Count_TimeTypeConvert(2,&Memory_tm1,p_User->EffectEndTime);
            ...
            if(i==(MEMORY_FLASH_USER_PERMISSION_GROUP_NUM-1))
            {
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_CARD_VISITOR,0);
            }
            else
            {
                Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_CARD_USER1+i,0);
            }
            sprintf((char*)Member_PrintBuf,"  获取:数量-%05d,校验-%05d,流水-%08ld\r\n",p_Check->Number,p_Check->CheckSum,p_Check->SerialNumber);
            DebugOut((int8_t*)Member_PrintBuf,strlen((char*)Member_PrintBuf));
        }
    ...
    if(cmd & Memory_TEST_WR_TYPE)
    {
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        //
        memset((uint8_t*)p_Ver,0x00,sizeof(S_MEMORY_VER));
        p=(uint8_t*)p_Ver;
        for(i=0; i<sizeof(S_MEMORY_VER); i++)
        {
            p[i]=i+1;
        }
        Memory_InternalApp(MEMORY_APP_CMD_VER_W,(uint8_t*)p_Ver,0,0);
        memset((uint8_t*)p_Ver,0x00,sizeof(S_MEMORY_VER));
        res = Memory_InternalApp(MEMORY_APP_CMD_VER_R,(uint8_t*)p_Ver,0,0);
        //比对
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        //
        memset((uint8_t*)p_MemoryCheck,0x00,sizeof(S_MEMORY_CHECK));
        p=(uint8_t*)p_MemoryCheck;
        for(i16=0; i16<sizeof(S_MEMORY_CHECK); i16++)
        {
            p[i16]=i16+1;
        }
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_W,(uint8_t*)p_MemoryCheck,MEMORY_APP_CHECK_ALL,0);
        memset((uint8_t*)p_MemoryCheck,0x00,sizeof(S_MEMORY_CHECK));
        res = Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_MemoryCheck,MEMORY_APP_CHECK_ALL,0);
        //比对
        ...
        p_Net=(S_MEMORY_NET*)MemberTestBuf;
        //
        sprintf((char *)Member_PrintBuf,"----------Memory Test Net Begin:\r\n");
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        //
        memset((uint8_t*)p_Net,0x00,sizeof(S_MEMORY_NET));
        p=(uint8_t*)p_Net;
        for(i=0; i<sizeof(S_MEMORY_NET); i++)
        {
            p[i]=i+1;
        }
        Memory_InternalApp(MEMORY_APP_CMD_NET_W,(uint8_t*)p_Net,0,0);
        memset((uint8_t*)p_Net,0x00,sizeof(S_MEMORY_NET));
        res = Memory_InternalApp(MEMORY_APP_CMD_NET_R,(uint8_t*)p_Net,0,0);
        //比对
        for(i=0; i<MEMORY_NET_SIZE-2; i++)
        {
            if(p[i]==(i+1));
            else break;
        }
        if(i==MEMORY_NET_SIZE-2 && MEMORY_NET_SIZE==sizeof(S_MEMORY_NET) && res==MEMORY_APP_ERR_NO)
        {
            sprintf((char *)Member_PrintBuf,"Memory Test Net Ok!\r\n");
            DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        }
        else
        {
            sprintf((char *)Member_PrintBuf,"Memory Test Net Err:%d\r\n",res);
            DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        }
        //
        sprintf((char *)Member_PrintBuf,"-----Memory Test Net End.\r\n");
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        sign_exe=TRUE;
        //---------------------控制
        p_Control=(S_MEMORY_CONTROL*)MemberTestBuf;
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        //
        memset((uint8_t*)p_Control,0x00,sizeof(S_MEMORY_CONTROL));
        p=(uint8_t*)p_Control;
        for(i=0; i<sizeof(S_MEMORY_CONTROL); i++)
        {
            p[i]=i+1;
        }
        Memory_InternalApp(MEMORY_APP_CMD_CONTROL_W,(uint8_t*)p_Control,0,0);
        memset((uint8_t*)p_Control,0x00,sizeof(S_MEMORY_CONTROL));
        res = Memory_InternalApp(MEMORY_APP_CMD_CONTROL_R,(uint8_t*)p_Control,0,0);
        //比对
        ...
        {
            memset((uint8_t*)p_Door,0x00,sizeof(S_MEMORY_DOOR));
            p=(uint8_t*)p_Door;
            for(j16=0; j16<sizeof(S_MEMORY_DOOR); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_DOOR_W,(uint8_t*)p_Door,i16,i16);
        }
        //
        for(i16=0; i16<MEMORY_DOOR_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_Door,0x00,sizeof(S_MEMORY_DOOR));
            res = Memory_InternalApp(MEMORY_APP_CMD_DOOR_R,(uint8_t*)p_Door,i16,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_DOOR,0);
        ...
        */
        ...
        {
            memset((uint8_t*)p_CardGBitmap,0x00,sizeof(S_MEMORY_CARDG_BITMAP));
            p=(uint8_t*)p_CardGBitmap;
            for(j16=0; j16<sizeof(S_MEMORY_CARDG_BITMAP); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_CARDGBITMAP_W,(uint8_t*)p_CardGBitmap,i16,i16);
        }
        //
        for(i16=0; i16<MEMORY_CARDGBITMAP_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_CardGBitmap,0x00,sizeof(S_MEMORY_CARDG_BITMAP));
            res = Memory_InternalApp(MEMORY_APP_CMD_CARDGBITMAP_R,(uint8_t*)p_CardGBitmap,i16,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_CARDG_BITMAP,0);
        ...
        {
            memset((uint8_t*)p_Reader,0x00,sizeof(S_MEMORY_READER));
            p=(uint8_t*)p_Reader;
            for(j16=0; j16<sizeof(S_MEMORY_READER); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_READER_W,(uint8_t*)p_Reader,i16,i16);
        }
        //
        for(i16=0; i16<MEMORY_READER_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_Reader,0x00,sizeof(S_MEMORY_READER));
            res = Memory_InternalApp(MEMORY_APP_CMD_READER_R,(uint8_t*)p_Reader,i16,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_READER,0);
        ...
        {
            memset((uint8_t*)p_Relay,0x00,sizeof(S_MEMORY_RELAY));
            p=(uint8_t*)p_Relay;
            for(j16=0; j16<sizeof(S_MEMORY_RELAY); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_RELAY_W,(uint8_t*)p_Relay,i16,i16);
        }
        //
        for(i16=0; i16<MEMORY_RELAY_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_Relay,0x00,sizeof(S_MEMORY_RELAY));
            res = Memory_InternalApp(MEMORY_APP_CMD_RELAY_R,(uint8_t*)p_Relay,i16,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_RELAY,0);
        ...
        {
            memset((uint8_t*)p_Input,0x00,sizeof(S_MEMORY_INPUT));
            p=(uint8_t*)p_Input;
            for(j16=0; j16<sizeof(S_MEMORY_INPUT); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_INPUT_W,(uint8_t*)p_Input,i16,i16);
        }
        //
        for(i16=0; i16<MEMORY_INPUT_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_Input,0x00,sizeof(S_MEMORY_INPUT));
            res = Memory_InternalApp(MEMORY_APP_CMD_INPUT_R,(uint8_t*)p_Input,i16,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_INPUT,0);
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        //
        memset((uint8_t*)p_Alarm,0x00,sizeof(S_MEMORY_ALARM));
        p=(uint8_t*)p_Alarm;
        for(i=0; i<sizeof(S_MEMORY_ALARM); i++)
        {
            p[i]=i+1;
        }
        Memory_InternalApp(MEMORY_APP_CMD_ALARM_W,(uint8_t*)p_Alarm,0,0);
        memset((uint8_t*)p_Alarm,0x00,sizeof(S_MEMORY_ALARM));
        res = Memory_InternalApp(MEMORY_APP_CMD_ALARM_R,(uint8_t*)p_Alarm,0,0);
        //比对
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_Check,MEMORY_APP_CHECK_ALARM,0);
        ...
        {
            memset((uint8_t*)p_DoorG,0x00,sizeof(S_MEMORY_DOOR_GROUP));
            p=(uint8_t*)p_DoorG;
            for(j16=0; j16<sizeof(S_MEMORY_DOOR_GROUP); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_DOORGROUP_W,(uint8_t*)p_DoorG,i16,i16);
        }
        //
        for(i16=0; i16<MEMORY_DOORGROUP_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_DoorG,0x00,sizeof(S_MEMORY_DOOR_GROUP));
            res = Memory_InternalApp(MEMORY_APP_CMD_DOORGROUP_R,(uint8_t*)p_DoorG,i16,0);
        ...
        {
            memset((uint8_t*)p_CardG,0x00,sizeof(S_MEMORY_CARDGROUP_LIMIT));
            p=(uint8_t*)p_CardG;
            for(j16=0; j16<sizeof(S_MEMORY_CARDGROUP_LIMIT); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_CARDGROUP_LIMIT_W,(uint8_t*)p_CardG,i16,i16);
        }
        //
        for(i16=0; i16<MEMORY_CARDGROUPLIMIT_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_CardG,0x00,sizeof(S_MEMORY_CARDGROUP_LIMIT));
            res = Memory_InternalApp(MEMORY_APP_CMD_CARDGROUP_LIMIT_R,(uint8_t*)p_CardG,i16,0);
        ...
        {
            memset((uint8_t*)p_Area,0x00,sizeof(S_MEMORY_AREA));
            p=(uint8_t*)p_Area;
            for(j16=0; j16<sizeof(S_MEMORY_AREA); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_AREA_W,(uint8_t*)p_Area,i16,i16);
            //读区域
            Memory_InternalApp(MEMORY_APP_CMD_AREA_R,(uint8_t*)p_Area,i16,i16);
            p[j16]=0;
        }
        //
        for(i16=0; i16<MEMORY_AREA_MAX_NUM; i16++)
        {
            memset((uint8_t*)p_Area,0x00,sizeof(S_MEMORY_AREA));
            res = Memory_InternalApp(MEMORY_APP_CMD_AREA_R,(uint8_t*)p_Area,i16,0);
        ...
        {
            memset((uint8_t*)p_LogicBlockInOutAnd,0x00,sizeof(S_MEMORY_LOGICBLOCK_INOUTAND));
            p=(uint8_t*)p_LogicBlockInOutAnd;
            for(j16=0; j16<sizeof(S_MEMORY_LOGICBLOCK_INOUTAND); j16++)
            {
                p[j16]=j16+1;
            }
            Memory_InternalApp(MEMORY_APP_CMD_LOGICBLOCK_INOUTAND_W,(uint8_t*)p_LogicBlockInOutAnd,i16,i16);
            //读区域
        ...
        {
            memset((uint8_t*)p_LogicBlockInOutAnd,0x00,sizeof(S_MEMORY_LOGICBLOCK_INOUTAND));
            res = Memory_InternalApp(MEMORY_APP_CMD_LOGICBLOCK_INOUTAND_R,(uint8_t*)p_LogicBlockInOutAnd,i16,0);
    ...
    if(cmd& Memory_TEST_WR_TIMEGROUP)
    {
        ...
        for(i16=0; i16<MEMORY_FLASH_MAX_TIMEGROUP; i16++)
        {
            ...
            //写入
            res = Memory_InternalApp(MEMORY_APP_CMD_TIMEGROUP_W,(uint8_t*)p,i16,i16);
            //清缓存
            for(j16=0; j16<sizeof(S_MEMORY_TIMEGROUP); j16++)
            {
                p[j16]=0;
            ...
            //读取
            res = Memory_InternalApp(MEMORY_APP_CMD_TIMEGROUP_R,(uint8_t*)p,i16,i16);
            //校验
    ...
    if(cmd& Memory_TEST_WR_HOLIDAYGROUP)
    {
        ...
        for(i16=0; i16<MEMORY_FLASH_MAX_HOLIDAYGROUP; i16++)
        {
            ...
            //写入
            res = Memory_InternalApp(MEMORY_APP_CMD_HOLIDAYGROUP_W,(uint8_t*)p,i16,i16);
            //清缓存
            for(j16=0; j16<sizeof(S_MEMORY_HOLIDAYGROUP); j16++)
            {
                p[j16]=0;
            ...
            //读取
            res = Memory_InternalApp(MEMORY_APP_CMD_HOLIDAYGROUP_R,(uint8_t*)p,i16,i16);
            //校验
    ...
    if(cmd& Memory_TEST_WR_CARDRECORD)
    {
        ...
        {
            //写
            memset((uint8_t*)p_CardRecord,0x01,sizeof(S_MEMORY_CARD_RECORD));
            ...
            res = Memory_InternalApp(MEMORY_APP_CMD_CARD_RECORD_W,(uint8_t*)p_CardRecord,0,0);
            if(res!=MEMORY_APP_ERR_NO)
            {
        ...
        {
            //读
            memset((uint8_t*)p_CardRecord,0,sizeof(S_MEMORY_CARD_RECORD));
            res = Memory_InternalApp(MEMORY_APP_CMD_CARD_RECORD_R,(uint8_t*)p_CardRecord,i32,0);
        ...
        p_MemoryCheck=(S_MEMORY_CHECK*)MemberTestBuf;
        //打印记录长度
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)p_MemoryCheck,0,0);
    ...
    if(cmd& Memory_TEST_WR_ALARMRECORD)
    {
        ...
        {
            //写
            memset((uint8_t*)p_AlarmRecord,0x01,sizeof(S_MEMORY_ALARM_RECORD));
            ...
            res = Memory_InternalApp(MEMORY_APP_CMD_ALARM_RECORD_W,(uint8_t*)p_AlarmRecord,0,0);
            if(res!=MEMORY_APP_ERR_NO)
            {
                sprintf((char *)Member_PrintBuf,"ID-%05ld Write Err!\r\n",i32);
                DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        ...
        {
            //读
            memset((uint8_t*)p_AlarmRecord,0,sizeof(S_MEMORY_ALARM_RECORD));
            res = Memory_InternalApp(MEMORY_APP_CMD_ALARM_RECORD_R,(uint8_t*)p_AlarmRecord,i32,0);
...
//-----读写用户权限测试(擦除+新建10000条+读取10000条,打印出每条读写占用的时间,经测试第10000张卡读写在116ms(4M)上下)
    if(cmd& Memory_TEST_WR_USER_NEW)
    {
        ...
        for(i16=0; i16<MEMORY_FLASH_MAX_USER; i16++)
        {
            ...
            memset((uint8_t*)p_User,0x55,sizeof(S_MEMORY_USER_PERMISSION));
            p_User->CardNumber[0]=0;
            p_User->CardNumber[1]=0;
            ...
            res = Memory_InternalApp(MEMORY_APP_CMD_NEW_USER_W,(uint8_t*)p_User,0,0);
            if(res==OK)
            {
                //逐条信息输出
                i32=Memory_msCount;
        ...
        if(res==OK && i16==1)
        {
            ...
            {
                i32=Memory_msCount=0;
                memset((uint8_t*)p_User,0x00,sizeof(S_MEMORY_USER_PERMISSION));
                p_User->CardNumber[0]=0;
                p_User->CardNumber[1]=0;
                p_User->CardNumber[2]=i16>>8;
                p_User->CardNumber[3]=i16;
                res = Memory_InternalApp(MEMORY_APP_CMD_USER_PERMISSION_R,(uint8_t*)p_User,0,0);
                if(res==OK)
                {
        ...
        if(res==OK && i16==1)
        {
            ...
            {
                i32=Memory_msCount=0;
                memset((uint8_t*)p_User,0,sizeof(S_MEMORY_USER_PERMISSION));
                p_User->Password[0]=0;
                p_User->Password[1]=0;
                p_User->Password[2]=(i16+1)>>8;
                p_User->Password[3]=(i16+1);
    ...
    if(cmd& Memory_TEST_WR_VISITOR_NEW)
    {
        ...
        for(i16=0; i16<MEMORY_FLASH_USER_PERMISSION_MAX_NUM; i16++)
        {
            ...
            p_User->CardNumber[2]=i16>>8;
            p_User->CardNumber[3]=i16;
            res = Memory_InternalApp(MEMORY_APP_CMD_NEW_VISITOR_W,(uint8_t*)p_User,0,i16);
            //逐条信息输出
            i32=Memory_msCount;
            sprintf((char *)Member_PrintBuf,"ID-%05d WTime:%ldms\r\n",i16,i32);
            DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        ...
        if(res==OK)
        {
            ...
            {
                i32=Memory_msCount=0;
                memset((uint8_t*)p_User,0xAA,sizeof(S_MEMORY_USER_PERMISSION));
                p_User->CardNumber[0]=0;
                p_User->CardNumber[1]=0;
                p_User->CardNumber[2]=i16>>8;
                p_User->CardNumber[3]=i16;
    ...
    if(cmd& Memory_TEST_READ_EVENT)
    {
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        //打印当前事件记录数量(i16)
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)&i16,MEMORY_APP_CHECK_CARD_RECORD,0);//事件记录更新
        ...
        {
            memset((uint8_t*)p_CardRecord,0,sizeof(S_MEMORY_CARD_RECORD));
            res = Memory_InternalApp(MEMORY_APP_CMD_CARD_RECORD_R,(uint8_t*)p_CardRecord,j16,0);
            ...
            Count_TimeTypeConvert(2,&Memory_tm,p_CardRecord->RecordTime);
    ...
    if(cmd& Memory_TEST_READ_ALARM)
    {
        ...
        DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        //打印当前事件记录数量(i16)
        Memory_InternalApp(MEMORY_APP_CMD_CHECK_R,(uint8_t*)&i16,MEMORY_APP_CHECK_ALARM_RECORD,0);//事件记录更新
        ...
        {
            memset((uint8_t*)p_AlarmRecord,0,sizeof(S_MEMORY_ALARM_RECORD));
            res = Memory_InternalApp(MEMORY_APP_CMD_ALARM_RECORD_R,(uint8_t*)p_AlarmRecord,j16,0);
            ...
            Count_TimeTypeConvert(2,&Memory_tm,p_AlarmRecord->RecordTime);
            sprintf((char *)Member_PrintBuf,"  Num-%05d Card-%05ld ID-%05d CH-%d Time-%04d-%02d-%02d %02d:%02d:%02d\r\n",
                    j16+1,i32,k16,p_CardRecord->ReaderOrWireOrRelayNum,Memory_tm.tm_year,Memory_tm.tm_mon+1,Memory_tm.tm_mday,Memory_tm.tm_hour,Memory_tm.tm_min,Memory_tm.tm_sec);
            DebugOut((int8_t*)Member_PrintBuf,strlen((char *)Member_PrintBuf));
        }
    ...
    if(cmd & Memory_TEST_READ_DOOR)
    {
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_DOOR_R,(uint8_t*)p_Door,i,0);
        ...
        */
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_CARDGBITMAP_R,(uint8_t*)p_CardGBitmap,i,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_READER_R,(uint8_t*)p_Reader,i,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_RELAY_R,(uint8_t*)p_Relay,i,0);
        ...
        {
            Memory_InternalApp(MEMORY_APP_CMD_INPUT_R,(uint8_t*)p_Input,i,0);
    ...
    if(cmd& Memory_TEST_WR_EEPROM)
    {
        ...
        for(k16=1; k16<=j16; k16++)
        {
            ...
            for(i32=0; i32<MEMORY_EERPOM_END_ADDR; i32+=MEMORY_FLASH_MAX_WRITE_SIZE)
            {
...
Goto_MemoryTest1:
    //释放缓存
    MemManager_Free(E_MEM_MANAGER_TYPE_256B,MemberTestBuf);
#ifdef OS_UCOSII
    OSSemPost(Sem_EepromFlashUse);
#else
#ifdef OS_UCOSIII
    OSSemPost(&Sem_EepromFlashUse,OS_OPT_POST_1,&err);
#endif
    i=0;
#endif
}
