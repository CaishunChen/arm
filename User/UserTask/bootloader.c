...
*********************************************************************************************************/
#define	PROJECT_NULL		0
#define	PROJECT_XKAP_V3		1
#define	XKAP_ICARE_B_D_M	2
#define	XKAP_GATEWAY		3
#define	XKAP_ICARE_A_M		4
#define	XKAP_ICARE_A_S		5
#define	XKAP_ICARE_A_C		6
#define	XKAP_ICARE_B_M		7
#define	XKAP_ICARE_B_C		8
#define	XKAP_ICARE_B_D		9
#define	PROJECT_NRF5X_BLE	10
#define	TEST_NRF52_V1		11
//硬件选择
#define	BOOT_PROJECT		PROJECT_NULL
//----------------------------------------------------------------------------
#if		(BOOT_PROJECT	==	PROJECT_NULL)
//#define USB_ENABLE
#elif	(BOOT_PROJECT	==	PROJECT_XKAP_V3)
#define	STM32F4
...
#elif	(BOOT_PROJECT	==	XKAP_ICARE_B_D_M)
#elif	(BOOT_PROJECT	==	XKAP_GATEWAY)
#elif	(BOOT_PROJECT	==	XKAP_ICARE_A_M)
#define	NRF51
#elif	(BOOT_PROJECT	==	XKAP_ICARE_A_S)
#define	NRF51
#elif	(BOOT_PROJECT	==	XKAP_ICARE_A_C)
#define	NRF51
#elif	(BOOT_PROJECT	==	XKAP_ICARE_B_M)
#define	NRF51
#elif	(BOOT_PROJECT	==	XKAP_ICARE_B_C)
#define	NRF51
#elif	(BOOT_PROJECT	==	XKAP_ICARE_B_D)
#elif	(BOOT_PROJECT	==	PROJECT_NRF5X_BLE)
#define	NRF51
#elif	(BOOT_PROJECT	==	TEST_NRF52_V1)
#define	NRF52
#define BOOTLOADER_TF_ENABLE
#define BOOTLOADER_KEY1_PIN     28
#endif
//----------------------------------------------------------------------------Includes
#include <string.h>
#include <stdio.h>
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if(BOOTLOADER_FileSize>BSP_CPU_FLASH_APP_ADDR_END+1-BSP_CPU_FLASH_APP_ADDR)
        {
            // 关闭文件
        ...
        //复制数据
        //擦除主程序区
        Bsp_CpuFlash_Erase(BSP_CPU_FLASH_APP_ADDR,BSP_CPU_FLASH_APP_ADDR_END);
        //将升级程序区(ISP_FLASH_ADDR_START_KB-ISP_FLASH_ADDR_END_KB)-->主程序区(20-USER_FLASH_ADDR_ADDR_KB)
        FLASH_Unlock();
        sum1=0;
        for(i32=BSP_CPU_FLASH_APP_ADDR; i32<BSP_CPU_FLASH_APP_ADDR_END; i32+=4)
        {
...
uint8_t check_isp_entry_pin(void)
{
#if		(defined(BOOTLOADER_TF_ENABLE)||defined(USB_ENABLE))
...
#elif   (defined(NRF51)||defined(NRF52))
	uint8_t res1=0;
#if		(defined(BOOTLOADER_KEY1_PIN))
    uint32_t pio=0;
    nrf_gpio_cfg_input(BOOTLOADER_KEY1_PIN, NRF_GPIO_PIN_PULLUP);
    pio=5;
    while(pio--)
    {
        __nop();
    }
    pio = nrf_gpio_pin_read(BOOTLOADER_KEY1_PIN);
    if(0!=pio)
    {
        res1=1;
    }
...
#elif   (defined(NRF51))
    nrf_bootloader_init();
    nrf_bootloader_app_start(0x0001B000);
#elif		(defined(NRF52))	
		nrf_bootloader_init();
