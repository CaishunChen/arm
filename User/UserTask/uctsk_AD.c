...
#include "nrf_delay.h"
#define NRF_DRV_SAADC_UASER_CHANNEL_CONFIG_SE(PIN_P) \
    {                                                  \
...
void ADC_Configuration(void)
{
    ...
    uint8_t i;
    //配置优先级
    nrf_drv_saadc_config_t saasc_config = {.resolution = SAADC_RESOLUTION_VAL_10bit,
                                           .oversample = NRF_SAADC_OVERSAMPLE_DISABLED,
                                           .interrupt_priority = 7,
                                           .low_power_mode = false
                                          };
    //配置中断函数
    err_code = nrf_drv_saadc_init(&saasc_config, saadc_callback);
    APP_ERROR_CHECK(err_code);

    nrf_saadc_channel_config_t channel_config_ain0 =
        NRF_DRV_SAADC_UASER_CHANNEL_CONFIG_SE(NRF_SAADC_INPUT_AIN0);
    nrf_drv_saadc_channel_init(0, &channel_config_ain0);
#endif
#endif		
    ...
    nrf_saadc_channel_config_t channel_config_ain2 =
        NRF_DRV_SAADC_UASER_CHANNEL_CONFIG_SE(NRF_SAADC_INPUT_AIN2);
    nrf_drv_saadc_channel_init(0, &channel_config_ain2);
#endif
...
{
#if (defined(XKAP_ICARE_A_S)||defined(XKAP_ICARE_A_C))
    static uint8_t si=1;
...
#elif	(defined(NRF52))
	static nrf_saadc_value_t i16_buf[5]= {0};
    uint8_t i=0;
    if(pValue!=NULL)
    {
        if(ch==0)
    ...
    if(OnOff==ON)
    {
        APP_ERROR_CHECK(nrf_drv_saadc_buffer_convert(i16_buf,5));
        for(i=0; i<5; i++)
        {
...
void AD_DebugTestOnOff(uint8_t OnOff)
{
    if(OnOff==ON)
    {
        ...
        {
            uint16_t i16;
            AD_ReadValue(ON,1,&i16);
            DebugOutHex("AD:", (uint8_t*)(&i16), 2);
        }
#endif
    }
    else
